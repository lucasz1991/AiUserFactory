...
>
    ...
    <div class="flex min-h-full items-center justify-center">
        <div x-show="show" class="relative z-10 flex max-h-[calc(100vh-3rem)] w-full flex-col overflow-hidden rounded-lg bg-white shadow-xl transform transition-all sm:max-h-[calc(100vh-6rem)] {{ $maxWidth }}"
            ...
            <div class="min-h-0 flex-1 overflow-y-auto scroll-container scroll-container-hover">
                {{ $slot }}
            </div>

            @isset($actions)
                <div class="flex shrink-0 flex-row flex-wrap justify-end gap-3 border-t border-gray-200 bg-gray-50 px-6 py-4 text-end">
                    {{ $actions }}
                </div>
            @endisset
        </div>
