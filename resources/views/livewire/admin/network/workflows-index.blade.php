<div class="space-y-6" wire:loading.class="opacity-60 pointer-events-none">
    ...
    <nav class="-mb-px flex gap-4 overflow-x-auto" aria-label="Workflow Gruppen">
        <button type="button" x-on:click="activeGroup = 'all'" wire:click="selectWorkflowGroup('all')" class="whitespace-nowrap border-b-2 px-1 py-3 text-sm font-semibold" :class="activeGroup === 'all' ? 'border-blue-600 text-blue-700' : 'border-transparent text-slate-500 hover:border-slate-300 hover:text-slate-700'">
            Alle
            <span class="ml-1 rounded-full bg-slate-100 px-2 py-0.5 text-xs text-slate-600">{{ $workflows->count() }}</span>
        </button>
        @foreach($groups as $group)
            <button type="button" x-on:click="activeGroup = '{{ $group }}'" wire:click="selectWorkflowGroup('{{ $group }}')" class="whitespace-nowrap border-b-2 px-1 py-3 text-sm font-semibold" :class="activeGroup === '{{ $group }}' ? 'border-blue-600 text-blue-700' : 'border-transparent text-slate-500 hover:border-slate-300 hover:text-slate-700'">
