...
function initMetisMenu() {
    ...
    if (window.MetisMenu) {
        sideMenu.__ffMetisMenu = new window.MetisMenu(sideMenu, { toggle: true });
        sideMenu.addEventListener('shown.metisMenu', () => setGroupAria(sideMenu));
        sideMenu.addEventListener('hidden.metisMenu', () => setGroupAria(sideMenu));
        return;
    }
    ...
    sideMenu.querySelectorAll('[data-ff-sidebar-group]').forEach((trigger) => {
        trigger.addEventListener('click', (event) => {
            ...
            item?.querySelector(':scope > ul')?.classList.toggle('mm-show', open);
    ...
    document.querySelectorAll('.vertical-menu-btn').forEach((button) => {
        ...
        button.addEventListener('click', (event) => {
            ...
            if (isDesktopSidebar()) {
                ...
                setMobileSidebarOpen(!appBody()?.classList.contains('sidebar-enable'));
            }
        });
    });
...
function bindGlobalShellInteractions() {
    ...
    document.addEventListener('livewire:navigate', clearSidebarTimers);
    document.addEventListener('livewire:navigating', () => setMobileSidebarOpen(false));
