...
| 2026-07-30 | Codex | verifiziert | **Finale Haertung der mobilen Workflow-Minimap.** Statische `status_routes` werden nur angezeigt, wenn keine allgemeinere `next`-, `on_partial`- oder `on_error`-Route mit Runtime-Vorrang greift. Auswaehlbare Minimap-Karten besitzen jetzt 44-Pixel-Touchziele, gueltige `aria-pressed`-Semantik sowie Enter-/Leertastensteuerung. | Abschliessender SQLite-In-Memory-Regressionspass: 87 Tests / 1.245 Assertions gruen. Blade-Cache und Vite-Produktionsbuild erfolgreich; bekannte Hinweise zu `icon-bonus.png` und grossem Pagebuilder-Chunk unveraendert. | 320-Pixel-Browser-Smoke-Test bei Gelegenheit ergaenzen; 390/768/1280 Pixel sind bereits authentifiziert abgenommen. |
