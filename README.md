...
| --- | --- | --- | --- | --- | --- |
| 2026-07-30 | Claude | in_arbeit | **Spur B — Marken-Relaunch FollowFlow (Icon, Schriftlogo, Favicon/PWA-Icons, animiertes GIF, Violett).** Beansprucht sind ausschliesslich Markenartefakte und deren direkte Einbindung: `public/site-images/brand/*`, `public/favicon.svg`, `public/favicon.ico`, `public/icons/*`, `public/site-images/favicon/*`, `public/manifest.webmanifest`, `app/Support/Pwa/PwaIcon.php`, `resources/views/components/branding/*`, `resources/views/components/navigation/application-icon.blade.php` sowie zeilengenaue Markenverweise in `resources/views/layouts/metahead.php`, `layouts/pwa-head.blade.php`, `layouts/topbar.blade.php` (nur Markentext/Logo) und `layouts/no-auth-layout.blade.php` (nur Fusszeilen-Markenname). **Abgrenzung zu Codex' App-Shell-Anspruch:** Shell-Geometrie, Sidebar, Topbar-Struktur, `app-shell.css/js`, Workflow-Views und gemeinsame UI-Komponenten bleiben unangetastet; in den Layouts werden nur Marken-Strings und Asset-Pfade getauscht, keine Klassen, keine Struktur. Runtime-, Routing-, Task-Katalog- und Push-Vertraege bleiben unveraendert (Icon-Dateinamen der PWA bleiben stabil). | In Arbeit. | Assets erzeugen (SVG-Vektor plus Pillow-Rasterung), Einbindung tauschen, PWA-/Markup-Tests und Vite-Build fahren. |
...
| 2026-07-30 | Codex | in_arbeit | **Bestehendes Workflow-Studio Browser-/DOM-Modal funktionell und visuell erweitern.** Beansprucht sind die gemeinsame `WorkflowRunPreview`-/`WorkflowStudio`-Oberflaeche, der vorhandene DOM-Inspector, die dafuer benoetigte redigierte Node-Runtime-Projektion und fokussierte Tests. Ziel: Body-DOM ohne Head, Selector-Suche mit mehreren gleichzeitigen Treffern in DOM und Screenshot, Elementauswahl per Screenshot, Elementdaten und mehrere kopierbare Selector-Vorschlaege sowie eine vertikale Anordnung mit Browseransicht oben und DOM darunter. Die Werkzeuge sollen waehrend eines pausierten bzw. nicht aktiv laufenden Testlaufs bedienbar bleiben. | In Arbeit; Architektur-, Datenvertrags- und Konfliktaudit laeuft. | Vorhandenen Datenpfad und Laufzustandsregeln pinnen, danach Runtime und gemeinsame Vorschau ohne zweite Modal-Implementierung erweitern. |
