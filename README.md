...
| 2026-07-30 | Codex | in_arbeit | **Task-Bibliothek und mobile Workflow-Uebersicht.** Vollstaendige Inventur der katalogisierten Node-/PHP-Tasks und ihrer Nutzung; fachliche Neugruppierung und Sortierung der Bibliothek; Wiederverwendung der bestehenden Workflow-Minimap im Editor mit diskreten Zoomstufen; mobile Optimierung von Katalog und Editor. Runtime-, Task-Key-, Routing-, Single-Step- und `next_task_key`-Vertraege bleiben unveraendert. Beansprucht: `app/Livewire/Admin/Network/WorkflowStudioTaskEditor.php`, `resources/views/livewire/admin/network/workflow-studio*.blade.php`, `resources/views/components/workflows/minimap.blade.php`, `resources/css/workflow-experience.css`, unmittelbar zugehoerige Tests und ein Auditbericht unter `.lmzdev/artifacts/reports/`. Die bereits unkommittiert geaenderten Dateien `workflow-manager.blade.php`, `workflows-index.blade.php` und `WorkflowCopilotUiMarkupTest.php` werden nicht angefasst. | Bestandsaufnahme und Kollisionspruefung laufen. | Katalog/Registry belegen, UI gezielt umsetzen, danach PHP-/Node-/Blade-/Build- und authentifizierte Responsive-Pruefung. |
