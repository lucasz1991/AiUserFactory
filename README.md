...
| Bereich | Soll | Ist |
| --- | --- | --- |
| Vorschau-Tests | Manuelle Tests und Copilot-Live-Optimierung verwenden dieselbe Oberflaeche mit Workflow-Karte, Steps, Tasks, Browserfenstern, Screenshots, Variablen, Artefakten und Logs. | Umgesetzt. Der aktive Einstieg oeffnet das eingebettete Workflow Studio; Diagramm und Laufprojektion bleiben ueber `x-workflows.run-preview` gemeinsam. Das fruehere separate Copilot-Vorschaumodal wird nicht mehr geoeffnet. |
...
| Chat-Scroll | Neue Nachrichten und Streaming-Text sollen automatisch folgen, solange der Benutzer am Chatende liest; beim Zurueckscrollen darf die Position nicht springen. | Umgesetzt. `MutationObserver` und `ResizeObserver` folgen nur im angehefteten Zustand. Beim Lesen aelterer Nachrichten bleibt die Position erhalten und `Neueste Aktivitaet` springt kontrolliert ans Ende. |
...
| Serverlokale Sprache | Followflow und RailTime sollen denselben abgesicherten Whisper-/Piper-Dienst ohne oeffentlichen Voice-Port verwenden koennen. | Followflow besitzt jetzt einen authentifizierten Loopback-Client fuer den gemeinsamen `/v1`-Dienst. Bei expliziter Aktivierung arbeiten beide Audio-Endpunkte fail-closed; die bisherige lokale CLI und die vorhandenen Provider bleiben nur bei deaktiviertem Shared-Service als bewusster Rollback erhalten. Die produktive Daemon-Aktivierung und der echte serverseitige Smoke-Test bleiben Teil des gemeinsamen Deployments. |
...
| `app/Services/Ai/SpeechServiceClient.php` | Authentifizierter, redirect-freier Loopback-Client fuer den gemeinsamen STT-/TTS-Dienst. |
...
| `docs/shared-speech-service.md` | Gemeinsamer Vertrag, sichere Aktivierung, Diagnose, fail-closed Verhalten und expliziter Rollback. |
...
| --- | --- | --- | --- | --- | --- |
| 2026-08-03 | Claude | verifiziert | **Zeitgesteuerte Personen-Workflows und automatisierte Personenerstellung (Pakete P1-P6).** Analyse und Plan liegen in `.lmzdev/artifacts/reports/personen-workflow-automatisierung-analyse.md` und `.lmzdev/artifacts/documents/personen-workflow-automatisierung-plan.md`. Kernbefund: `workflows.trigger_type` wird gespeichert, revisioniert und in Prompts mitgegeben, aber **nirgends ausgewertet**; in `Kernel::schedule()` startet **kein einziger Eintrag je einen Workflow**; `workflow_runs` hat **keine `person_id`-Spalte**, der Personenbezug liegt nur in `context_json`. Umfang: (P1) drei additive Migrationen — neue Tabelle `person_workflow_schedules`, `workflow_runs.person_id` als indizierter Spiegel samt Backfill, `persons.max_concurrent_workflow_runs` — plus `WorkflowBrowserBinding`, das browsergebundene Workflows rekursiv ueber eingebettete Workflows erkennt (Katalog trennt 24 `browser`, 8 `input`, 18 `data`, 5 `wait`). (P2) `PersonWorkflowDispatcher` mit zehnstufiger Pruefkette, Artisan-Befehl `network:dispatch-person-workflows`, Kernel-Eintrag jede Minute; Start ausschliesslich ueber den vorhandenen `WorkflowExecutionService::start(..., requestedBy: 'schedule')`. Browsergebundene Laeufe beanspruchen Browserprofil, Cookie-Datei und Session der Person exklusiv, damit die frei konfigurierbare Nebenlaeufigkeit keine Sessions zerschiesst. (P3) neuer Menuepunkt Netzwerk -> Automatisierung. (P4) Profil-Tab Automatisierung mit echter Lauf-Historie je Person. (P5) neuer Menuepunkt Netzwerk -> Personen-Fabrik: Bauplaene erzeugen Personen als **Entwuerfe**, Aktivierung erst nach manueller Freigabe. (P6) der vorhandene Sandbox-Aktivitaetsplan wird als zweite Taktquelle wirksam. Beansprucht: `app/Services/Automation/*`, `app/Services/Persons/*`, neue Models/Migrationen/Jobs/Commands, `app/Console/Kernel.php`, `app/Livewire/Admin/Network/{Automation,PersonFactory}*`, Profil-Tab in `person-detail.blade.php`, `routes/web.php`, `layouts/sidebar.blade.php` und neue Tests. **Regel 2 gewahrt** (keine neuen Task-Keys, der Dispatcher startet vorhandene Workflows), **Regel 7 nicht betroffen** (`node/workflows` bleibt unangetastet). Additiv in `WorkflowExecutionService::start()` nur das Spiegeln von `person_id`. | SQLite in-memory: **73 Tests / 434 Assertions** gruen (`PersonWorkflowSchedule`, `PersonWorkflowDispatcher`, `PersonFactory`, dazu die Bestandsfilter `PersonAccountRegistry`, `PersonProfilePage`, `PersonProfileMarkup`, `LivewireSubdirectory`, `WorkflowRuntimeFingerprint`, `Authentication`). Die 34 neuen Tests pinnen die Taktberechnung je Quelle (Intervall, feste Uhrzeiten mit Tagesuebergang, Aktivitaetsplan mit Session-Filter), das Zeitfenster **in der Ortszeit der Person** (Fall `America/New_York`), Wochentagssprung, Kontext-Bereinigung gegen Geheimnisse und fremdes `person_id`, jede Stufe der Pruefkette einzeln, die Browser-Erkennung inklusive eingebetteter Workflows und Zyklus, sowie die Fabrik (Entwurf nie sofort aktiv, Freigabe legt Zeitplaene an, Zielzahl schaltet ab, Not-Aus loescht nichts). Migrationen gegen SQLite und die lokale MySQL-Instanz ausgefuehrt. Pint, `php -l`, `npm run build`, `view:cache` und `git diff --check` sauber; `node/workflows` blieb unangetastet (Regel 7 nicht ausgeloest). **Echter Ende-zu-Ende-Lauf ueber den Artisan-Befehl:** Zeitplan angelegt, `network:dispatch-person-workflows` startete den Lauf (`1 faellig, 1 gestartet`), der Lauf traegt `person_id=1` und `requested_by=schedule`, `next_run_at` rueckte um 60 Minuten vor, `runs_today=1`, und die Historie war ueber die neue Spalte abfragbar. Der Wiederholungslauf lehnte korrekt mit *„Person faehrt bereits 1 Laeufe"* ab. Authentifizierter Browser-Smoke-Test beider neuer Seiten (Status 200, Hero, KPI-Reihe, Tabs mit genau einem sichtbaren Panel, kein horizontaler Ueberlauf, beide Menuepunkte in der Sidebar) und des neuen Profil-Tabs *Automatisierung* mit Ortszeit-Anzeige. **Ein eigener Fehler wurde dabei gefunden und behoben:** `computeNextRunAt()` gab zunaechst UTC zurueck; Eloquent serialisiert Datumswerte aber als Wanduhrzeit und liest sie in `config('app.timezone')` wieder ein — der Zeitplan lief dadurch dauerhaft eine Stunde zu frueh. Testartefakte, temporaerer Admin und Testzeitplan wurden entfernt; Automatisierung und Fabrik stehen bewusst auf **aus**. | **Vor der Inbetriebnahme pruefen, ob auf der Produktion ein Cron jede Minute `php artisan schedule:run` ausfuehrt** — ohne ihn startet der Dispatcher nie, und der Queue-Worker muss laufen, sonst bleiben die Laeufe in `queued`. Danach in Netzwerk -> Automatisierung einschalten. Offen als moegliche Erweiterung: Zeitplaene direkt aus dem Profil heraus anlegen (heute verlinkt der Tab auf die Netzwerkseite) und mehrere Zeitplan-Vorlagen je Bauplan statt einer. |
| 2026-08-03 | Claude | verifiziert | **Personen-Profil: zwei echte Darstellungsfehler behoben, einer davon appweit.** Nach der Abnahme meldete der Nutzer das Profil als "komplett zerschossen". Die Sichtpruefung mit dem projekteigenen Puppeteer (echter Login, Vollseiten-Screenshot) zeigte zwei unabhaengige Ursachen. **(1) Appweit — Livewire unter Unterverzeichnis-Installation:** `FrontendAssets::js()` und `HandleRequests::getUpdateUri()` bauen `/livewire/livewire.js` und `/livewire/update` ab Domainwurzel. Lokal laeuft die Anwendung unter `http://localhost/followflow/AiUserFactory/public`, dort liefert `livewire.js` **404**. Damit startet auch Alpine nicht (es kommt in diesem Projekt mit Livewire), und die Seite rendert **alle sieben Tab-Panels untereinander** — 6497 px statt 1223 px. `AppServiceProvider` haengt jetzt den echten `Request::getBaseUrl()` vor beide URLs; bei leerem Basispfad (Produktion, `artisan serve`) aendert sich nichts. Betrifft jede Livewire-Seite, nicht nur das Profil. **(2) Selbst verursacht — geliehene Workflow-Klasse:** meine Primaerknoepfe trugen `ff-action-trigger--primary`, nur um den Magnet-Effekt aus `workflow-motion.js` zu bekommen. Diese Klasse setzt in `workflow-experience.css` ihren Verlauf per `!important` und greift dabei auf `var(--ff-ink)` zu, das nur im Workflow-Bereich definiert ist. Ausserhalb wird die Kurzschreibweise ungueltig, `background-color` faellt auf `transparent` zurueck — der Knopf war **weisse Schrift auf weissem Grund**. Ersetzt durch den eigenen Haken `data-magnetic`. Zusaetzlich: alle 42 `var(--ff-profile-*)` haben jetzt Ersatzwerte, die Kontenleiste wird ab der zweispaltigen Ansicht nicht mehr auf 32 rem abgeschnitten, und `x-init` setzt ein Bereitschaftsflag, damit ein JavaScript-Ausfall kuenftig eine saubere Uebersicht zeigt statt aller Panels uebereinander. | SQLite in-memory: **49 Tests / 390 Assertions** gruen (`PersonAccountRegistry`, `PersonProfileMarkup`, `PersonProfilePage`, `LivewireSubdirectoryAssetPath`, `WorkflowRuntimeFingerprint`, `PwaFrontend`, `Authentication`). Neue Regressionen pinnen genau die drei Fehler: keine geliehene Workflow-Knopfklasse, Panel-Ausfallsicherung, `var()` ohne Ersatzwert. Sichtpruefung mit echtem Login als temporaerem Konto (danach `forceDelete`, kein Auth-Bypass im Repo) gegen **beide** Umgebungen: Apache-Unterverzeichnis und `artisan serve` — jeweils Alpine und Livewire aktiv, genau ein Panel sichtbar, aktiver Tab in Markenblau `rgb(47, 92, 158)`, Livewire-Roundtrip beim Kontowechsel erfolgreich, Primaerknopf jetzt `rgb(47, 92, 158)` statt `rgba(0, 0, 0, 0)`. Vollseiten-Screenshots bei 1440/820/390 px ohne horizontalen Ueberlauf. Pint und `php -l` sauber. **Verbleibend und vorbestehend:** unter dem Apache-Unterverzeichnis liefern die in `icons.min.css` wurzelrelativ referenzierten Icon-Schriften weiterhin 404 (`/build/icons/...`); das betrifft das Vendor-CSS der Admin-Vorlage, nicht diese Spur. | Falls die Icon-Schriften unter dem Unterverzeichnis stoeren, denselben Weg wie hier waehlen und die Pfade in `icons.min.css` relativ aufloesen. Ansonsten kein Nachlauf. |
| 2026-08-03 | Claude | verifiziert | **Personen-Profil neu konzipiert und Accounts vereinheitlicht.** (1) `person-detail.blade.php` wird komplett neu aufgebaut: die Kopfleiste verliert `Zurueck`, `Timeouts`, `Session aufbauen` und `Bilder` (alle vier bleiben an ihrer fachlich richtigen Stelle im Accounts- bzw. Medien-Tab erreichbar), neuer Profil-Header, neu gesetzte KPI-Reihe und neue Tab-Ordnung. (2) Die Tabs **E-Mail** und **Social Media** werden zu einem Tab **Accounts** zusammengefuehrt; darin stehen E-Mail-Konto, Instagram, Facebook, X, TikTok und weitere Portale in einer Liste. (3) Neue kanonische Kontenquelle `App\Services\Persons\PersonAccountRegistry` und darauf ein neues Livewire-Paar `Admin\Config\PersonAccounts`. (4) Workflows erreichen die Kontendaten neu ueber die Punktstruktur `person.accounts.<typ>.username|address|password` (plus `provider`, `url`, `hasPassword`) fuer jeden Kontotyp. Beanspruchte Bereiche: `resources/views/livewire/admin/config/person-detail.blade.php`, `app/Livewire/Admin/Config/PersonDetail.php`, die neuen Accounts-Dateien, `resources/css/person-profile.css` + `resources/js/components/person-profile-motion.js` (GSAP, additiv wie `workflow-motion.js`) samt je einer Importzeile in `app.css`/`app.js`, sowie **additiv** `WorkflowExecutionService::workflowRuntimeContext()`, die Redaktionsliste in `WorkflowTaskRunner`, die Wertegruppen in `WorkflowTaskCatalog` und die Redaktion `publicWorkflow()` in `node/workflows/run_step.cjs` (Regel 6 + Regel 7). Nicht angefasst: Task-Keys, Routing, Copilot, `PersonEmailAccountSettings` (wird im neuen Tab unveraendert weiterverwendet). Hinweis nach Regel 9: der Arbeitsbaum enthielt bei Uebernahme fremde, nicht committete Aenderungen an `WorkflowTaskCatalog.php`, `WorkflowTaskRunner.php` und `run_step.cjs`; meine Ergaenzungen dort sind rein additiv und heben nichts davon auf. | SQLite in-memory: **18 Tests / 164 Assertions** gruen fuer `PersonAccountRegistry`, `PersonProfilePage` und `PersonProfileMarkup`; im breiteren Filter zusaetzlich `WorkflowCollectionTaskCatalog`, `WorkflowTaskFormMarkup`, `WorkflowRuntimeFingerprint`, `WorkflowSelectorSyntax` und `PersonaActivityPlanner` gruen. Node: neuer Test `person.accounts paths resolve per account type and their passwords stay out of the public status` gruen (Aufloesung je Kontotyp, unbekannter Typ -> `missing_context_value`, Passwoerter im oeffentlichen Status leer). Pint, `php -l`, `npm run build`, `php artisan view:cache` und `git diff --check` sauber. Runtime-Sync nach Regel 7 ausgefuehrt (`npm run sync:workflow-runtime`), `run_step.cjs` bytegleich, Manifest-Hash `51df1b8cde7438424046d690bc6cd0247b9ea66a0bc97b727e8dd968a72587a8`. **Authentifizierter Browser-Smoke-Test** am laufenden Server (Port 8721): Hero mit 18px-Radius und Verlauf, sechs KPI-Karten, sieben Tabs mit 44px-Touchzielen, Tabwechsel blendet genau ein Panel ein, Accounts-Tab listet alle zwoelf Kontotypen, Instagram-Karte bietet die fuenf Aktionen inklusive `Timeouts`, und der Knopf `Timeouts` oeffnet ueber die neue Alpine-Bruecke wirklich den Modal des Elternkomponenten; keine Consolefehler, kein horizontaler Ueberlauf. **Ein echter Fund aus dem Smoke-Test wurde behoben:** der Panelwechsel animierte zuerst `autoAlpha`; bei gedrosseltem GSAP-Ticker blieb der Startzustand `opacity: 0` stehen und der gerade geoeffnete Tab war leer. Der Tween animiert jetzt ausschliesslich `transform`. **Zwei vorbestehende Fehlschlaege ohne Bezug zu dieser Spur** (beide aus der parallel laufenden Workflow-Assistance-Arbeit, gegen `e18f23f` gegengeprueft): `WorkflowTaskLibraryPresentationTest` erwartet 48 Katalog-Tasks, es sind durch die drei neuen Assistance-/Human-Tasks 51; `WorkflowTaskLibraryMarkupTest` findet zwei erwartete Markup-Stellen im Definition-Editor nicht mehr. Ebenso vorbestehend: fuenf Routing-/Embedded-Tests in `run_step.test.cjs` schlagen auch mit dem unveraenderten Runner aus `e18f23f` fehl. | Kein funktionaler Nachlauf. Fuer weitere Kontotypen genuegt ein Eintrag in `PersonAccountRegistry::TYPES` — Oberflaeche, Workflow-Kontext und Task-Editor ziehen automatisch nach. Offen bleibt, ob das Instagram-Passwort dauerhaft nur ueber den Zugangsdaten-Dialog laufen soll; sobald die Base-Verschluesselung aus `PersonList::saveProfile()` extrahiert ist, koennte auch dieses Feld in den Accounts-Tab wandern. |
| 2026-08-01 | Codex | verifiziert | **Followflow an den gemeinsamen Speech-Dienst angebunden.** Der neue `SpeechServiceClient` spricht den finalen Loopback-Vertrag auf `127.0.0.1:8092` mit separater Client-ID und Bearer-Token beziehungsweise serverseitiger Token-Datei. STT sendet exakt `audio_base64`, `filename`, `mime_type`, `language`; TTS exakt `text`, `speed`. URLs, Client-ID, Token-Header, MIME-Typ und WAV-Antwort werden validiert, Redirects und ausgehende Proxys sind gesperrt. Bei `SPEECH_SERVICE_ENABLED=true` verwenden beide bestehenden Audio-Endpunkte ausschliesslich den Shared-Service und fallen bei Fehlern nicht auf lokale oder externe Provider zurueck; bei `false` bleiben die bisherigen Provider und die lokale Whisper-/Piper-CLI unveraendert als expliziter Rollback. Neue getrennte Rate Limits, sicherer Status-/Rundtestbefehl, Betriebsdokumentation und leere Credential-Platzhalter in `.env.example`; der erneut vorhandene `/tests/`-Ignore wurde entfernt, damit neue Tests versioniert werden. RailTime und der Daemon blieben in diesem Track read-only. | SQLite in-memory: **57 Tests / 328 Assertions** fuer Shared-Service, vorhandene Speech-/Voice-Provider, Chatbot-Markup und Chatbot-Hardening gruen. Pint, PHP-Syntax, reale Routen-/Middleware-Aufloesung und `git diff --check` erfolgreich. Der Vertrag wurde gegen die parallel entstandene Daemon-Dokumentation und Implementierung (`status=ready|degraded`, Engine-Strings, Port 8092) gegengeprueft. | Gemeinsamen Daemon samt eigenem Followflow-Token unter Supervisor deployen, produktive `.env` aktivieren und `php artisan assistant:speech-service:status --smoke` ausfuehren. Zuvor versionierte Credential-Werte kontrolliert rotieren; `APP_KEY` nur mit geplantem Re-Encryption-/Session-Verfahren und Git-History nur nach ausdruecklich koordiniertem Rewrite behandeln. |
| 2026-08-07 | Claude | verifiziert | **Sichtbarer JavaScript-Code ueber dem Header der Workflow-Ansicht behoben.** In `resources/views/livewire/admin/network/workflow-manager.blade.php` stand das Alpine-Objekt in einem `x-data="..."`-Attribut, dessen Ausdruck selbst doppelte Anfuehrungszeichen enthielt: `querySelectorAll('.jetstream-modal, [role="dialog"][aria-modal="true"]')` (Zeile 143) und `querySelector('[data-workflow-mobile-library][data-open="true"]')` (Zeile 153). Fuer den HTML-Parser endet das Attribut bereits bei `[role="` — die restlichen rund 3000 Zeichen JavaScript landeten als **sichtbarer Textknoten** direkt unter der Ueberschrift „WORKFLOW MANAGEMENT", als direktes Kind von `div.workflow-experience`. Im Quelltext ist das unauffaellig, weil Blade den Ausdruck nicht escaped und der Fehler erst im gerenderten Markup entsteht. Fix: CSS-Attributselektoren unquotiert geschrieben (`[role=dialog][aria-modal=true]`, `[data-open=true]`) — gueltiges CSS, identische Semantik, keine Anfuehrungszeichen mehr im Attribut. Bewusst **nicht** geaendert: die zweite Auffaelligkeit derselben Seite — `resources/views/components/branding/animated-mark.blade.php` gibt sein CSS per `@once` als nacktes `<style>` aus, das dadurch **innerhalb** des `<a class="ff-topbar-brand__link">` landet (HTML-ungueltig, aber `display:none` und damit unsichtbar). Sauber waere `@once` + `@push('styles')`, da `layouts/master.blade.php:21` einen `@stack('styles')` bereitstellt; das ist ein eigener Fix und war nicht Teil des Auftrags. | Ursache im Browser belegt: vor dem Fix trug ein Textknoten unterhalb von `div.workflow-experience` 3058 Zeichen JavaScript, `innerText` zeigte `dialog !== shell && this.elementIsVisible(dialog));…` als Zeile 16 der Seite. Nach dem Fix: `x-data`-Attribut vollstaendig mit 3235 Zeichen und korrektem Ende, **null** sichtbare Code-Zeilen, **null** Code-Textknoten. Neuer Regressionstest `tests/Unit/BladeAlpineAttributeIntegrityTest.php` prueft **alle** Blade-Dateien auf vorzeitig beendete `x-`/`wire:`/`@`-Attribute (unbalancierte Klammern oder Abbruch im Attributselektor); Wirksamkeit per Gegenprobe bewiesen — mit wieder eingebautem Bug **rot** samt Datei:Zeile, ohne **gruen**. Repo-weiter Scan nach demselben Muster: keine weiteren Fundstellen. `git check-ignore` bestaetigt, dass der neue Test versioniert wird (bekannte `/tests/`-Falle). | **Nicht committet.** Die volle Testsuite ist in dieser Umgebung nicht aussagefaehig: 368 Tests brechen mit `Access denied for user 'forge'@'localhost'` ab, weil `.env.testing` keine `DB_`-Eintraege hat und die entsprechenden Zeilen in `phpunit.xml` auskommentiert sind — **vorbestehend**, per `git stash` auf HEAD ohne meine Aenderung reproduziert. Wer die Suite fahren will, setzt `DB_CONNECTION=sqlite DB_DATABASE=:memory:` davor, wie in den frueheren Eintraegen. Offen: der `<style>`-im-`<a>`-Fund oben. |
| 2026-07-31 | Claude | verifiziert | **Workflow-Studio: Listen und Tasks direkt in der Testlauf-Oberflaeche bearbeitbar.** Bisher ging der Aufbau nur im Manager, weil der geteilte Editor `resources/views/livewire/admin/network/partials/workflow-definition-editor.blade.php` Bibliothek, Canvas und die Dialoge fuer *Neue Liste*, *Liste bearbeiten*, *Task einsetzen* und *Task verschieben* per `@unless($modalOnly)` vollstaendig ausgeblendet hat; im Studio blieb nur das Task-Bearbeiten-Modal uebrig. Diese Bloecke haengen jetzt an `$showDefinitionSurface` und werden im Studio in eine Overlay-Huelle (`data-studio-definition-drawer`, `fixed inset-0 z-50`) gerendert, die ueber der dichten Vollbildansicht liegt und wieder geschlossen werden kann; Werkzeugleiste, Browserfenster, Timeline und Copilot-Rail bleiben unveraendert. Neu in `WorkflowStudioTaskEditor`: `showDefinitionDrawer`, Listener `open-workflow-studio-definition-drawer`, `closeDefinitionDrawer`; die teure Routenkarte wird weiterhin nur berechnet, wenn der Canvas wirklich sichtbar ist. `WorkflowStudio::openDefinitionBuilder()` prueft `assertUserControl`, uebergibt die aktuell gewaehlte Liste bzw. Task und wird nur im interaktiven Zweig der Kommandoleiste angeboten (Knopf **Listen & Tasks**); autonome Sitzungen oeffnen das Overlay nicht und schliessen es bei einem Moduswechsel automatisch, zusaetzlich zur bestehenden `canEdit`-Sperre. Die Kind-Dialoge tragen jetzt instanzbezogene IDs, damit Manager und eingebettetes Studio gleichzeitig im DOM stehen koennen. Es kommen nur Klassen aus dem bestehenden CSS-Build vor (kein `z-[75]`, kein neuer Opacity-Wert), damit die Huelle auch ohne Vite-Rebuild richtig stapelt. Der Manager-Pfad (`modalOnly=false`) rendert unveraendert. Nebenbei: `x-show` am Verbindungslinien-Umschalter auf `x-show.important` gezogen, weil `inline-flex` mit Tailwind-`important` sonst gewinnt. | `DB_CONNECTION=sqlite DB_DATABASE=:memory: php artisan test --filter="WorkflowStudio\|WorkflowCopilotUiMarkup\|WorkflowUi\|WorkflowTaskLibraryMarkup"` -> 62 gruen, 546 Assertions, **1 vorbestehender Fehlschlag**: `WorkflowCopilotUiMarkupTest` Zeile 152 sucht `x-show.important` in `workflow-manager.blade.php`, das Markup liegt seit der Editor-Extraktion im geteilten Partial; der Test ist auch auf HEAD rot und wurde nicht angefasst. Regression Manager-Pfad: `--filter="WorkflowTaskLibraryMarkup\|WorkflowComposition\|WorkflowRouteMarkup\|WorkflowTaskFormMarkup\|WorkflowTouchReorderMarkup\|WorkflowDomInspectorMarkup"` -> 36 gruen. Neue Tests: `tests/Feature/WorkflowStudioTest.php` (Overlay oeffnen, Liste anlegen, Katalog-Task einsetzen, Verschachtelung von Bibliothek und Canvas per XPath, autonome Sperre) und `tests/Unit/WorkflowStudioDefinitionDrawerMarkupTest.php`. `vendor/bin/pint` und `php -l` auf den geaenderten PHP-Dateien sauber. | Nicht committet. Browser-Smoke-Test des Overlays steht aus (kein temporaerer Dev-Login angelegt, weil parallel mit `git add -A` committet wird). Der vorbestehende Fehlschlag in `WorkflowCopilotUiMarkupTest` bleibt offen. |
| 2026-07-30 | Claude | verifiziert | **Spur B — Icon-Auslieferung fuer Tab und installierte App geprueft, zwei Maskenfehler behoben.** Die Verweise standen bereits (Favicon SVG/ICO, Apple-Touch, PWA-Route, Manifest), aber die Rasterung hatte einen echten Mangel: **alle** Icons trugen die abgerundete Badge-Form mit durchsichtigen Ecken. Beim `maskable`-Icon legt Android jedoch seine eigene Maske an — die freien Ecken scheinen dann durch; und iOS hinterlegt Transparenz im Apple-Touch-Icon mit Schwarz und rundet selbst ab. `render_mark()` kennt jetzt `square_badge`: `pwa-maskable-512.png` (Innenabstand 0.62) und beide Apple-Touch-Icons (0.86) fuellen das Quadrat randlos in `#2E1065`, die `purpose=any`-Icons behalten die eigene Rundung. Zusaetzlich `?v=2026-08-01` an den Favicon-, ICO- und Apple-Touch-Verweisen in `metahead.php` und `pwa-head.blade.php` — Browser halten Favicons in einem eigenen Cache und ignorieren dafuer gewoehnliche Neuladen. **Im Manifest bewusst ohne Query:** `PwaFrontendTest` prueft die Icon-Namen ueber `basename()`, eine Query wuerde diesen Vertrag brechen. | 21 Tests / 90 Assertions gruen (`PwaFrontend\|PwaIcon\|FactoryWebPush`), `view:cache` gruen. Am laufenden Server: alle sieben Icon-URLs mit 200 und korrektem MIME, auch mit Query; ausgelieferte `pwa-192.png` byteweise identisch mit der Datei (SHA-256), Mittelpunkt `rgb(143,97,246)`. Eckdeckkraft je Datei geprueft: maskable und Apple-Touch jetzt 255 in `#2E1065`, `any` weiterhin 0. Alle vier ICO-Eintraege (16/32/48/64) einzeln ausgelesen und auf hellem wie dunklem Grund angesehen. | Nach dem Deploy einmal hart neu laden (Strg+F5) und eine bereits installierte PWA neu installieren — das Startsymbol wird bei der Installation eingebrannt und folgt keinem Manifest-Update. |
| 2026-07-30 | Claude | verifiziert | **Spur B — Nachzug aus der Abnahme: Topbar dauerhaft animiert, echtes Schriftlogo auf der Markenseite.** Das Zeichen in der Topbar lief nur bei Hover; `components/navigation/application-icon.blade.php` hat jetzt einen `mode`-Prop mit Vorgabe `always` und reicht ihn an `animated-mark` durch — die Hover-Fassung bleibt fuer andere Flaechen verfuegbar. Die Markenseite der Anmeldung zeigte den Namen als gesetzten Text (`h1` plus `p`) statt der Wortmarke; neu liefert der Generator `followflow-wordmark{,-light}.svg` — dieselben Quicksand-700-Pfade wie das Lockup, aber ohne Badge, weil das Zeichen darueber bereits gross steht. Die Markenseite bindet die helle Fassung als Bild im `h1` ein (Alternativtext `FollowFlow — AI User Factory`), die Formularseite behaelt das vollstaendige Lockup in der dunklen Fassung. **Falle dabei:** die erste Fassung nutzte `h-[4.6rem]`; solche Arbitrary-Werte stehen erst nach einem Tailwind-Build im ausgelieferten `tailwind.min.css`, die Wortmarke fiel deshalb auf ihre Eigengroesse zurueck. Jetzt `h-20`. | 28 Tests / 154 Assertions gruen (`PwaFrontend\|PwaIcon\|Authentication\|AppShellMarkup`), `npm run build` und `view:cache` gruen. Am laufenden Server bei 1280 px gemessen: Zeichen 112x112 mit `border-radius 28%` und laufendem GIF, Wortmarke 276x80, beide auf Panelmitte 844 zentriert (843/844), Formularlogo 163x44 auf der weissen Seite; Topbar-Komponente isoliert geprueft und traegt `ff-anim-mark--always`. Gegengeprueft, dass `1.6rem`, `h-20`, `h-28`, `fuchsia-500` und die drei Verlaufsfarben im gebauten CSS stehen. | Unveraendert: `APP_NAME=FollowFlow` in der Produktions-`.env` nachziehen; authentifizierte Sichtpruefung der Topbar bei 390/768/1280 px steht weiter aus (Browser-Panel ohne Compositing). |
| 2026-07-30 | Claude | verifiziert | **Spur B — Marken-Relaunch FollowFlow (Icon, Schriftlogo, Favicon/PWA-Icons, animiertes GIF, Violett).** Die Marke heisst jetzt durchgaengig **FollowFlow · AI User Factory** statt „Factory AI · User Factory". Neues Zeichen: leuchtende KI-Sphaere in Violett (#8B5CF6 Kern, #3B0F73 Rand) auf tiefviolettem Badge (#2E1065 → #110626), umflossen von zwei Bahnen mit Partikeln; Schriftlogo in Quicksand 700 mit Verlauf #7C3AED → #C026D3 auf „Flow". **Alle Artefakte entstehen aus einer Quelle:** `scripts/brand/generate-brand-assets.py` (Pillow + fontTools) erzeugt SVG, PNG, ICO und GIF aus derselben Geometrie — Wortmarke als echte Pfade, damit sie ohne installierte Schrift identisch bleibt. Neu/ersetzt: `public/site-images/brand/followflow-{mark,mark-plain,logo,logo-light}.svg`, `followflow-logo{,-light}.png`, `followflow-mark-animated.gif` (128 px, 30 Bilder, 72 kB, gemeinsame Palette ohne Dithering), `public/favicon.svg`, `public/favicon.ico` (16/32/48/64 — die Datei war zuvor **0 Byte**), `public/icons/*`, `public/site-images/favicon/*`, beide Manifeste (Theme `#7C3AED`, Hintergrund `#110626`). `app/Support/Pwa/PwaIcon.php` zeichnet den Notfall-Fallback jetzt als dieselbe Sphaere mit weicher Kante statt der alten Teal-Saeulen; `BACKGROUND` ist oeffentlich, damit der Test keine Farbliterale doppelt. Neue Komponente `components/branding/animated-mark.blade.php`: statisches SVG als Traeger, GIF deckungsgleich darueber (Topbar bei Hover/Fokus, Auth-Marke dauerhaft), `prefers-reduced-motion` schaltet es ab. `application-logo` liefert zusaetzlich eine helle Fassung fuer den Dunkelmodus und reicht die uebergebene Klasse endlich durch (`$class` war nie deklariert und fiel still weg). **Bestandsfehler mitbehoben:** `layouts/metahead.php` traegt die Endung `.php`, wurde also von der PHP- statt der Blade-Engine gerendert — die vier Icon-/Manifest-Verweise standen woertlich als `{{ asset(...) }}` im Markup und waren wirkungslos. Jetzt `<?= e(asset(...)) ?>`; der dort doppelte, nicht installierbare `rel="manifest"` ist entfallen (pwa-head bleibt die einzige Quelle). **Abgrenzung zu Codex' App-Shell-Anspruch:** in `layouts/*` wurden ausschliesslich Marken-Strings und Asset-Verweise getauscht — keine Klassen, keine Geometrie, keine Struktur. `@persist('factory-ai-sidebar')` bleibt bewusst stehen (interner Schluessel, kein sichtbarer Text). Runtime-, Routing-, Task-Katalog- und Push-Vertraege unveraendert; die PWA-Icon-Dateinamen bleiben stabil, damit Manifest und Push-Payload weiter greifen. | SQLite in-memory: **37 Tests / 194 Assertions** gruen fuer `PwaIcon\|PwaFrontend\|FactoryWebPush\|PushSubscriptionApi\|AppShellMarkup`, zusaetzlich 43 gruen im Filter `Authentication\|PasswordReset\|ProfileInformation\|Markup`. `php artisan view:cache`, `npm run build` (nur die bekannten Hinweise zu Browserslist und Pagebuilder-Chunk), `php -l` und Pint gruen. Ausgeliefertes Markup am laufenden Server geprueft: Titel `FollowFlow · AI User Factory`, Fusszeile, Markenseite `FollowFlow` / `AI User Factory`, `theme-color #7C3AED`, alle acht Marken-URLs mit 200 und korrektem MIME; Auth-Zeichen 112x112 mit sichtbarem GIF (`opacity 1`, `border-radius 28%`), Logo-Umschaltung hell/dunkel ueber `data-mode` live bestaetigt; Topbar-Komponente isoliert ueber `Blade::render` geprueft. Icon-Dateien gegengelesen (Groessen und Modus je PNG, vier ICO-Eintraege). **Zwei vorbestehende Fehlschlaege ohne Bezug zu dieser Spur:** `RegistrationTest` erwartet 404 fuer `/register`, bekommt 302 (Route leitet auf `/login` um), und `WorkflowCopilotUiMarkupTest` bricht am Workflow-Manager-Markup aus Codex' laufender Arbeit. **Restrisiko:** kein Pixel-Screenshot — das Browser-Panel lieferte in dieser Sitzung kein Compositing, die Abnahme erfolgte ueber DOM, Computed Styles und Auslieferung. | `APP_NAME=FollowFlow` in der Produktions-`.env` nachziehen (lokal und `.env.example` sind gesetzt, `.env` deployt nicht mit) — sonst heisst die installierte PWA weiter `AiUserFactory`. Danach die authentifizierte Sichtpruefung von Topbar-Hover und Dunkelmodus bei 390/768/1280 px nachholen. |
| 2026-07-30 | Claude | verifiziert | **Spur K — Copilot-Optimierungskonzept umgesetzt (Bausteine E, A1, B, D, F).** Grundlage ist die Ursachenanalyse an 31 Produktionssitzungen; Zielbild und Ablaufplan liegen als `docs/Copilot-Konzept-v2.pdf`, Entscheidungen in `.lmzdev/DECISIONS.md`, Aufgabentafel in `.lmzdev/TASKS.md` (Spur K). **E — Erfolgskriterien:** Freitext-Musterset um den Rueckgabewert ergaenzt (der Auswertungstyp `workflow_return_type` existierte, war aber nur ueber strukturierte Keys erreichbar; die strikte Bestandsregel kannte weder `workflow_return` noch erklaerenden Zusatz). Neu `isCheckableCriterion()` plus zeilenweise Live-Rueckmeldung im Startdialog — inklusive Warnung, wenn kein einziges Kriterium pruefbar ist, weil `evaluateSuccessCriteria()` bei `total=0` vakuum-wahr `pass=true` meldet. **A1 — Selektor-Qualitaet:** `WorkflowSelectorSyntaxService::qualityWarningsFor()` erkennt sprachabhaengige Textreffer, fluechtige Attribute und zu allgemeine Elementtyp-plus-Text-Selektoren; `auditTaskCards()` prueft gespeicherte Workflows. Nicht blockierende Live-Hinweise im Task-Editor. Am echten Workflow verifiziert: zwei Befunde an den Consent-Selektoren. **B — Portal-Profile:** neue Tabelle `workflow_portal_profiles` (MySQL-sicher ueber `selector_hash` char(64), Unique auf domain+role+hash), Model und `WorkflowPortalProfileService` mit `remember`, `recordMiss`, `bestFor`, `rankedSelectorsFor`, `forgetStale`, Domain-Normalisierung. Verfallene Eintraege (mehr Misses als Hits) werden nicht geliefert; warnungsfreie Selektoren schlagen belastete bei gleicher Quote. **D — Zielfortschritt:** `goal_progress` wurde bisher nur protokolliert; jetzt wird ein Rueckschritt als `repair.progress_regressed` erfasst und nach `max_progress_regressions` (Default 3) sauber beendet. **F — Gezielte Rueckfrage:** neues additives Event `assistance.requested` mit Frage, Step/Task, Screenshot-Bezug und bis zu fuenf Kandidaten-Elementreferenzen, statt still zu pausieren. Bewusst NICHT umgesetzt: A2 (Selektor-Check gegen das echte DOM) und C/R (Erkundungsmodus, Rezeptableitung) — sie aendern `node/workflows` und loesen den ClientController-Runtime-Sync nach Regel 7 aus. | Gefilterte Regression ueber alle betroffenen Bereiche: **179 Tests / 1392 Assertions gruen** (`--filter="WorkflowCopilot|WorkflowAssistant|WorkflowUi|WorkflowManager|WorkflowSelector|WorkflowPortal"`), darunter neu `WorkflowSelectorQualityTest` (8), `WorkflowPortalProfileTest` und `WorkflowCopilotProgressAndAssistanceTest`. Blade-Cache, `php -l` und Pint gruen. Bestandsfixes vom selben Tag nachweislich intakt (`checkpoint_repairs_total`, Fingerprint ohne `current_revision`, `isCheckableCriterion`). Ein vollstaendiger Suite-Lauf bricht reproduzierbar am 40-Sekunden-PHP-Limit der lokalen Umgebung ab — vorbestehend, das Team faehrt laut Protokoll ohnehin gefiltert. | Migration auf der Zielumgebung ausfuehren (`php artisan migrate`), danach A2 angehen: Selektor-Check gegen das echte DOM samt Runtime-Sync. Portal-Profile sind gebaut, aber noch nicht an Beobachtung und Reparatur verdrahtet — dieser Anschluss ist der naechste inhaltliche Schritt. |
| 2026-07-30 | Codex | verifiziert | **Bestehendes Workflow-Studio Browser-/DOM-Modal funktionell und visuell erweitert.** Der Inspector zeigt pro Browserfenster den Screenshot oben und einen optisch verdichteten DevTools-Baum darunter. Er verarbeitet ausschliesslich das `body`, durchsucht den Snapshot per CSS-Selector, `text=`, `:has-text()` oder Klartext, markiert mehrere Treffer gleichzeitig im Baum und als Screenshot-Overlays und navigiert zwischen ihnen. Ein Klick in den Screenshot waehlt den kleinsten passenden bedienbaren DOM-Knoten, scrollt ihn in die Baumansicht und zeigt sichere Attribute, Zustand, Geometrie sowie mehrere bewertete, kopierbare Selector-Vorschlaege. Snapshot-Suche und -Inspektion bleiben in jedem Laufzustand verfuegbar; echte Browser-Probes sind serverseitig fail-closed und nur bei exakt manuell pausiertem, eigenem Lauf erlaubt. Fensteridentitaeten werden ueber `targetId`/`windowKey` abgeglichen, damit DOM und Screenshot bei mehreren Fenstern nicht falsch kombiniert werden. Die redigierte Node-Runtime serialisiert keine Werte oder Eventattribute und erzeugt Selector-Vorschlaege ohne teure Abfragen pro Knoten. | **15 PHP-Tests / 119 Assertions**, **11 Node-Tests** und **2 Rust-Fingerprint-Tests** gruen; PHP-/Node-Syntax, Blade-Cache, AiUserFactory- und ClientController-Produktionsbuild erfolgreich. Die drei geaenderten Runtime-Dateien stimmen bytegenau mit dem ClientController-Bundle ueberein; Server-Fingerprint `a6d7855e55d47dcd3b0f2cd9832a270156acf2c723083f5d7ad224b5019a97e9`. Realer Chromium-Benchmark: 1001/1001 Knoten in 149 ms, 419.043 Bytes, nicht gekuerzt. Authentifizierter Browser-Smoke-Test bestaetigte zwei gleichzeitige `button`-Treffer, Screenshot-Auswahl des zweiten Buttons, DOM-Fokus, Selector-Kopie und reine Textsuche ohne neue Inspector-Consolefehler. | Kein funktionaler Nachlauf fuer diesen Auftrag. Der bereits bekannte globale Alpine-Parserfehler ausserhalb des Inspectors bleibt eine getrennte Folgearbeit; einen echten pausierten externen Browserlauf bei der naechsten Workflow-Endabnahme nochmals pruefen. |
| 2026-07-30 | Codex | verifiziert | **Task-Bibliothek verfeinert, unbedienbare Task-Tabs repariert und task-spezifische Selector-Syntaxpruefung ergaenzt.** Die Bibliothek besitzt nun einen klaren Header, kompakte horizontal scrollbarere Kurzgruppen, lesbarere Task-Zeilen mit Typ-Icons, Trefferstatus und einen aussagekraeftigen Leerzustand. Der mobile Workflow/Bibliothek-Umschalter ist mit Panel-IDs und `aria-controls` verknuepft. Die gemeinsame Tab-Komponente schneidet Tabs nicht mehr per `overflow-hidden` ab, sondern stellt sie als 44-Pixel-Touchziele in einer horizontalen Scrollleiste dar; Tailwinds globales `important: true` kann inaktive Panels dank `x-show.important` nicht mehr sichtbar halten. Pfeil-, Home- und End-Tasten steuern den Roving-Tabindex. Aus den produktiven Node-Task-Skripten wurden drei unterschiedliche Eingabevertraege abgeleitet: erweiterte Resolver-Ziele (CSS/Text und priorisierte Alternativen), reines Chromium-CSS sowie strukturierte JSON-Selectoren bzw. Workflow-Variablennamen. Eine neue gemeinsame Blade-/Alpine-Komponente validiert alle betroffenen Primaer- und Zusatzfelder live, markiert Fehler direkt, zeigt ueber ein kleines `?` task-passende Beispiele und wird beim Speichern durch dieselbe PHP-Regel serverseitig abgesichert. `role=` wird bewusst nicht versprochen; ARIA-Rollen werden als CSS wie `[role=button]` angegeben. Runtime-Task-Keys, Runner, Skripte und Routing blieben unveraendert. | 14 Laravel-Unit-Tests / 228 Assertions, 19 Node-/Resolver-Tests und 3 SQLite-In-Memory-Studio-Featuretests / 23 Assertions gruen. PHP- und Node-Syntax, `git diff --check`, Blade-Cache sowie Vite-Produktionsbuild erfolgreich. Authentifizierte Browser-Abnahme auf Workflow 3: Bibliotheksfilter, Task-Modal, Live-Fehler/Erfolg, `?`-Hilfe, serverseitiges Blockieren und Pfeiltastenwechsel bestaetigt; nach einem Serverfehler erscheint die Meldung genau einmal. Der Runtime-Paritaetsfall `text=What's new` ist explizit gepinnt. Bekannte Build-Hinweise zu `/site-images/icon-bonus.png` und dem grossen Pagebuilder-Chunk sind unveraendert. | Kein funktionaler Nachlauf fuer diesen Auftrag. Bei kuenftigen Task-Skripten mit neuen Selector-Feldern den Feldvertrag zentral in `WorkflowSelectorSyntaxService::FIELD_MODES` ergaenzen und mit einem Node-/PHP-Paritaetstest pinnen. |
| 2026-07-30 | Claude | verifiziert | **Spur D — Copilot-Optimierungslauf: Ursachenanalyse und drei Schleifenfixes.** Grundlage sind die echten Produktionsdaten (31 Sitzungen, 8440 Events, 505 Runs, per SSH lesend erhoben): **kein einziger Lauf war je erfolgreich** (0x `verification.passed`, 0x `session.completed`), `budget.exceeded`=0, jede Sitzung endete durch **manuellen Abbruch**; zusammen 14,9 h Laufzeit bei 2,93 USD. Nur 5 von 31 Sitzungen erreichten die Endpruefung ueberhaupt; die uebrigen 26 blieben in `repair.paused` (6), `repair.plan_rejected_duplicate` (3), `repair.no_progress` (3) und `queue.supervisor_failed` (3) haengen. Die letzten sieben Pruefungslaeufe waren technisch einwandfrei (`completed`/`ok`/`technical_status=success`) und scheiterten allein an `business_status=failed` — fachliche Ursache laut Protokoll: zu allgemeine Selektoren (`button:has-text("Decline")` technisch erfolgreich, fachlich wirkungslos). Behoben wurden die drei Selbstblockaden, die dem Copilot die Mittel zur Selbstkorrektur nahmen: **(1)** `WorkflowCopilotRepairService::trustedVisionElementRefs()` verwarf bei `verdict='pause'`/`safe_pause` **pauschal alle** Vision-Elementreferenzen — genau die Datengrundlage fuer einen besseren Selektor. Jetzt entscheidet die ohnehin strengere Einzelpruefung (im DOM beobachtet, sichtbar, bedienbar, eigener Konfidenzwert, sicherer Selektorkandidat); nur die unbrauchbare Gesamtbeobachtung schliesst noch pauschal aus. **(2)** `repairPlanFingerprint()` enthielt `current_revision`, die jede Reparatur selbst erhoeht — derselbe wirkungslose Plan galt dadurch stets als neu, die Duplikaterkennung griff praktisch nie; zusaetzlich bekamen `pause`-Plaene gar keinen Fingerprint. Revision entfernt (Zustandsgleichheit entscheidet weiterhin die `state_signature`), `pause` einbezogen. **(3)** Der Checkpoint-Reparaturpfad hatte **kein** Iterationsbudget: `repair_iterations` wird ausschliesslich bei technischen Fehllaeufen gezaehlt, und der Signaturzaehler faellt bei wechselnder Fehlerstelle auf 0 zurueck. Neuer, nie zuruecksetzender Zaehler `checkpoint_repairs_total` mit eigenem Budgetschluessel `max_checkpoint_repairs` (Default 15) — bewusst **getrennt** von `max_repair_iterations`, dessen Semantik (technische Fehllaeufe) ein erster Versuch faelschlich ueberladen hatte, was zwei Bestandstests aufdeckten. Zusaetzlich ein Erklaer-PDF unter `docs/Copilot-Optimierungslauf.pdf` mit Ist- und Soll-Prozessdiagramm. Geaendert: `WorkflowCopilotSupervisorService.php`, `WorkflowCopilotRepairService.php`, neu `docs/Copilot-Optimierungslauf.pdf`. | Baseline vor der Aenderung 82 Tests/617 Assertions gruen — dabei zunaechst 82 Fehlschlaege aufgetreten, Ursache war ein unvollstaendiger `vendor`-Stand nach Codex' `composer.lock`-Update (mit `composer install` synchronisiert, `composer.lock` selbst unangetastet). Nach den Fixes: **148 Tests/1224 Assertions gruen** im Copilot-/Assistant-Umfang, davor 162/1367 inklusive Chatbot-Suite; `php -l` und Pint fuer beide Dateien gruen. Zwei zwischenzeitliche Fehlschlaege stammten aus dem Budgetschluessel-Konflikt und sind mit der Trennung behoben. Restrisiko: kein realer End-to-End-Lauf gegen eine echte Seite — die Wirkung der Fixes ist statisch und per Test belegt, nicht produktiv beobachtet. | Einen echten Optimierungslauf mit vorab geprueften, sprachunabhaengigen Selektoren fahren und pruefen, ob `checkpoint_repairs_total` sauber greift. Offen und **nicht** Teil dieser Spur: die Endpruefung nutzt denselben Vision-Prompt wie die Laufzeitbeobachtung (`verdict='pass'` ist dort nur ein nacktes Enum) und `evaluateSuccessCriteria()` meldet bei null Assertionen `pass=true`; beides ist latenter Zweitblocker, sobald Laeufe fachlich durchkommen. Erfolgskriterien sollten zudem bei Eingabe validiert werden — produktiv genutzte Werte wie `sassion speichern` fallen still auf `unsupported`. |
| 2026-07-30 | Codex | verifiziert | **Finale Haertung der mobilen Workflow-Minimap.** Statische `status_routes` werden nur angezeigt, wenn keine allgemeinere `next`-, `on_partial`- oder `on_error`-Route mit Runtime-Vorrang greift. Auswaehlbare Minimap-Karten besitzen jetzt 44-Pixel-Touchziele, gueltige `aria-pressed`-Semantik sowie Enter-/Leertastensteuerung. Der veraltete Collection-Test bildet den automatisch angehaengten Browser-Session-Speichertask nun korrekt ab. | Abschliessender SQLite-In-Memory-Regressionspass: 87 Tests / 1.245 Assertions sowie Node-Collection-/Loop-Suite 18/18 gruen. Blade-Cache und Vite-Produktionsbuild erfolgreich. Authentifizierter Browser bei 320 px: kein horizontaler Modal-Overflow, alle 11 Minimap-Tasks mindestens 44 px hoch; Zoom 144/192/224 px. Bekannte Hinweise zu `icon-bonus.png` und grossem Pagebuilder-Chunk unveraendert. | Kein funktionaler Nachlauf fuer diesen Auftrag; der bestehende globale Alpine-Parserfehler bleibt eine getrennte Folgearbeit. |
| 2026-07-30 | Codex | verifiziert | **UI-Regressionspass fuer Sidebar, Dropdowns, Workflow-Header und Audio.** Die mobile Sidebar ist geschlossen nicht mehr fokussierbar, verwaltet Fokus und ARIA-Zustand, behaelt den Desktop-Zustand bei `wire:navigate` und zeigt im kompakten Modus keine leeren Abschnittsbaender. Das gemeinsame Dropdown wird per Alpine-Teleport und Anchor aus scrollbaren Tabellen geloest, bleibt im Viewport, unterstuetzt Tastatursteuerung und korrekte Menue-Semantik. Workflow-Manager und -Liste clippen ihre Header-Menues nicht mehr; bedingte Flex-Elemente verwenden wegen `important: true` jetzt `x-show.important`. Beim Chat-Audio sind der 80-Nachrichten-Replay, Stop/Neustart-Races, doppelte manuelle Queue-Eintraege sowie Aufnahme-/STT-Leaks beim Destroy behoben; das Sprachmenue ist auch in niedrigen Viewports scrollbar. Runtime-, Routing-, Task-Katalog- und Single-Step-Vertraege blieben unveraendert. | 15 UI-/Markup-Tests mit 276 Assertions sowie 50 Workflow-Studio-/Chatbot-Hardening-/Speech-Tests mit 296 Assertions gruen. `node --check resources/js/app-shell.js`, Blade-Cache, Vite-Produktionsbuild und `git diff --check` erfolgreich. Bekannte Build-Hinweise: veraltete Browserslist-Daten, zur Laufzeit aufgeloestes `icon-bonus.png`, grosser Pagebuilder-Chunk. | Authentifizierten visuellen Smoke-Test fuer erstes/letztes Workflow-Zeilenmenue, Header-Menues, Mobile-Drawer und Sprachmenue bei 320/390/768/1024/1440 px nachholen; der lokale authentifizierte Browserpfad wurde in diesem Lauf durch die URL-Policy blockiert. |
| 2026-07-30 | Codex | verifiziert | **Task-Bibliothek und mobile Workflow-Uebersicht abgeschlossen.** Alle 48 Registry-Keys, 49 produktiven Node-Task-Skripte, 10 Helper sowie die parallelen PHP-Handler wurden inventarisiert. Kein verwaistes produktives Node-Skript ist gefahrlos loeschbar. 43 fachliche Tasks sind zentral und stabil in `Start & Navigation`, `Finden & Auslesen`, `Interagieren & Eingeben`, `Warten & Entscheiden`, `Schleifen & Listen`, `Mail & Sitzungen` sowie `Daten & Abschluss` sortiert; dynamische Unter-Workflows erhalten eine eigene Gruppe. `browser.open_browser_session`, `loop.end`, `browser.highlight`, `data.save_workflow_data` und `data.persist_browser_session` bleiben vollstaendig ausfuehrbar, sind als technische/automatische Bausteine aber aus der normalen Bibliothek ausgeblendet. Die gemeinsame Workflow-Minimap steht jetzt bereits vor einem Run im Task-Editor bereit, zeigt konfigurierte Routen, fokussiert Task-Karten und bietet die semantischen Zoomstufen `Uebersicht`, `Standard`, `Detail` (144/192/224 px). Mobil und auf Tablets wird zwischen Workflow und Task-Bibliothek umgeschaltet; Katalogsuche und Gruppenfilter funktionieren global. Runtime-`kind`, Runner, Task-Key, Skriptpfad, Routing, Single-Step und `next_task_key` blieben unveraendert. Vollbericht: `.lmzdev/artifacts/reports/task-library-audit-2026-07-30.md`. | SQLite-In-Memory: 117 fokussierte Tests / 1.410 Assertions gruen. `php artisan view:cache`, PHP-Syntax fuer Katalog/Manager/Studio-Editor und `npm run build` erfolgreich. Authentifizierte Browserpruefung auf Workflow 3 bei 390, 768 und 1280 px: Desktop-Gruppen und globale Suche, mobile Workflow/Katalog-Umschaltung, statische Routen sowie alle Zoomstufen visuell und semantisch bestaetigt; live gemessene Kartenbreiten 144/192/224 px und korrekte `aria-pressed`-Zustaende. | Separat behandeln: Der bestehende Node-Test `collection_tasks.test.cjs` erwartet noch nicht den automatisch angehaengten Task `__automatic-browser-session-save` (17/18 gruen). Ausserhalb des neuen Editorflusses erscheint beim Vollseitenladen ein globaler Alpine-Parserfehler `Unexpected token 'if'`; beim Oeffnen und Bedienen des Editors entstand kein zusaetzlicher Konsolenfehler. |
| 2026-07-30 | Claude | verifiziert | **Spur C — Chatbot-Analyse und Bugfixes.** Vier-Linsen-Tiefenanalyse des Workflow-Assistant-Chatbots mit adversarialer Gegenpruefung jedes schweren Befunds; neun Befunde erhoben, acht bestaetigt, einer widerlegt und bewusst **nicht** gepatcht. Behoben: **(1)** XSS im Statusstrom — `stream('assistant-status-stream', ...)` gab den modellgelieferten Toolnamen ohne `e()` aus, Livewire schreibt Streams per `innerHTML`; zusaetzlich gibt der `default`-Zweig von `assistantToolStatus()` den Fremdnamen nicht mehr aus. **(2)** Stored XSS ueber `copilotScreenshotUrl()` — die node-gelieferte `latest_screenshot_url` landete ungeprueft als `href`; `safeUrl()` blockt nur `wss`/`cdp`, nicht `javascript:`. Jetzt Schema-Allowlist (absolute http(s)-URL oder projektinterner Pfad, kein `//host`), sonst greift der Artefakt-Fallback. Dieselbe Stelle in `WorkflowManager.php` mitgehaertet (Datei war unbelegt). **(3)** `trimTranscript()` schnitt mit `take(-36)` rein positional und liess verwaiste `role=tool`-Antworten ohne ihren `assistant`-Aufruf stehen — OpenAI-kompatible Anbieter lehnen das Transcript dann ab, und da es in der Session persistiert, blieb der Chat dauerhaft unbenutzbar. **(4)** Leere Streaming-`tool_call`-IDs (`AiConnectionService` legt `'id' => ''` an) griffen den `??`-Fallback nicht mehr. **(5)** Steuerbefehl-Erkennung war unverankert: eine ausformulierte Anweisung wie „…lass den Browser kurz anhalten" wurde als `pause` gewertet, sofort und ohne Rueckfrage ausgefuehrt, die Anweisung selbst verworfen — jetzt Wortgrenze (`COPILOT_CONTROL_MAX_WORDS`), interne `__copilot_*`-Token bleiben ausgenommen. **(6)** `appendToolEvent()` legte Roh-Argumente und komplette Tool-Ergebnisse in eine public Property, die im Livewire-Snapshot und per `@entangle` in Alpine landet, ohne je gerendert zu werden. **(7)** `x-show` ohne `.important` bei `important: true` in Tailwind — leeres rotes Audio-Fehlerbanner und abgewaehlte Optionsbuttons blieben sichtbar. **(8)** `observeMessages()` wurde nur im `init` gebunden; startet die Seite mit geschlossenem Panel, fehlte `$refs.messages` und der Chat folgte keiner Streaming-Antwort mehr. Zusaetzlich TTS-Object-URLs sofort freigegeben (Speicherleck bei Auto-Vorlesen) und ein redundanter `fresh()`-Query je Poll-Tick entfernt. **Widerlegt und nicht umgesetzt:** fehlendes `#[Locked]` auf `activeCopilotSessionId` — es gibt keine Mandanten- oder Besitzergrenze, alle Routen liegen hinter `role:admin`, und `attachCopilotSession()` ist selbst eine public Action; `#[Locked]` haette nichts verhindert. Beansprucht und geaendert: `app/Livewire/Tools/Chatbot.php`, `resources/views/livewire/tools/chatbot.blade.php`, `app/Livewire/Admin/Network/WorkflowManager.php` (nur `copilotScreenshotUrl`), neu `tests/Feature/ChatbotHardeningTest.php`. **Zusatzbefund ausserhalb der Spur, sofort behoben:** Commit `65f05da` hatte `/tests/` in `.gitignore` aufgenommen, ohne Testdateien zu entfernen. Bereits getrackte Tests blieben getrackt, jeder **neue** Test war jedoch unsichtbar — betroffen waren `ChatbotHardeningTest` (Spur C), `FactoryWebPushNotificationTest`, `PushSubscriptionApiTest`, `PwaFrontendTest` (Spur W) und `AppShellMarkupTest` (App-Shell). Lokal liefen sie weiter, der Verlust waere erst im frischen Checkout oder in der CI aufgefallen. Zeile entfernt, alle fuenf Dateien sind wieder versionierbar; Details und Pruefkommando in `.lmzdev/messages/outbox.md`. Der im selben Commit gesetzte Eintrag `.env.productions` (mit „s") wurde bewusst **nicht** angefasst und zur Gegenpruefung gemeldet. **Abgrenzung zu Codex' App-Shell-Anspruch vom selben Tag:** ich fasse weder `resources/views/layouts/*`, noch App-Shell-CSS/JS (`app-shell.css`, `app-shell.js`, `app.js`), noch Workflow-Views (`workflow-manager.blade.php` u. a.), noch gemeinsame UI-Komponenten an — Codex hat diese Dateien aktuell unkommittiert in Arbeit. **Abgrenzung zu Spur W (PWA/Push):** keine Beruehrung, disjunkte Dateimengen. Runtime-, Routing-, Task-Katalog- und Force-Stop-Vertraege bleiben unveraendert; das Auditlog bleibt append-only. | Baseline vor der Aenderung: `DB_CONNECTION=sqlite DB_DATABASE=:memory: php artisan test --filter="WorkflowCopilot\|WorkflowAssistant\|AssistantSpeech"` → 158 Tests / 1268 Assertions gruen (bewusst vor dem Patchen erhoben, weil Codex' Shell-Umbau unkommittiert im selben Arbeitsbaum liegt und Fremdfehler sonst nicht unterscheidbar waeren). Nach der Aenderung: derselbe Filter erweitert um `WorkflowManager\|WorkflowUi\|Composition\|ChatbotHardening` → **187 Tests / 1464 Assertions gruen**, keine Regression. Neu `tests/Feature/ChatbotHardeningTest.php` mit 11 Tests / 33 Assertions, die jeden Fix einzeln pinnen — jeder davon steht vor dem zugehoerigen Patch nachweislich rot (verwaiste Tool-Antwort, leere Streaming-ID, Freitext-Fehltreffer, Rohdaten im Snapshot, unescapter Statusstrom, `javascript:`-Schema, fehlendes `.important`, ungebundener Observer). `php artisan view:cache` und `php -l` fuer beide geaenderten PHP-Dateien gruen. Restrisiko: kein authentifizierter Browser-Smoke-Test — die drei Frontend-Fixes (Fehlerbanner, Optionsbuttons, Autoscroll) sind per Markup- und Regex-Test gepinnt, aber nicht visuell abgenommen. | Beim naechsten Browserlauf die drei Frontend-Fixes visuell bestaetigen. Offen und **nicht** von Spur C abgedeckt: `WorkflowCopilotObservationService::safeUrl()` nutzt weiterhin eine Blockliste (`wss`/`cdp`) statt einer Schema-Allowlist — die Haertung erfolgte an den beiden Senken, nicht an der Quelle; ein dritter Konsument wuerde die Luecke erneut oeffnen. |
| 2026-07-30 | Codex | in_arbeit | Vollstaendiges App-Shell-Redesign der AiUserFactory nach der read-only Referenz `C:\xampp\htdocs\RailTime\App`: Sidebar, Topbar, gemeinsame Oberflaechen und responsive Inhaltsgeometrie werden nativ in Laravel/Livewire/Alpine/Tailwind adaptiert. Workflow-Liste, Manager, Studio/Testlauf und Run-Preview bleiben innerhalb derselben Shell; Runtime-, Routing-, Chatbot- und Force-Stop-Vertraege werden nicht veraendert. Beansprucht: `resources/views/layouts/*`, ausgewaehlte gemeinsame UI-Komponenten, App-Shell-CSS/JS, Workflow-Views, UI-Tests, README und `.lmzdev`. RailTime bleibt unveraendert. | Quell-, Layout- und Browser-Audit laeuft. | Referenzmuster kartieren, Shell adaptieren, responsive Breakpoints und authentifizierte Routen testen. |
| 2026-07-27 | Claude | verifiziert | High-End-Politur des gemeinsamen Workflow-/Copilot-Designsystems auf dem Codex-Stand vom 2026-07-27, rein Frontend: `workflow-experience.css` vollstaendig ueberarbeitet (selbst gehostete Plus Jakarta Sans Variable nur fuer Workflow-Flaechen, Double-Bezel ueber konzentrische Schattenringe ohne DOM-Aenderung, dunkle Glas-Inseln mit Glow-Orbs fuer Step-Header/Status-/Run-Islands/Copilot-Header/User-Bubbles, mehrschichtige weiche Ambient-Schatten, federnde `cubic-bezier(.32,.72,0,1)`-Uebergaenge, Kicker als Pill-Eyebrow). Neues additives GSAP-Modul `resources/js/components/workflow-motion.js` (gsap 3.15.0 via npm): IntersectionObserver-Reveals mit Stagger ueber `gsap.utils.distribute`, Metrik-Count-ups, magnetische Primaerbuttons ueber `gsap.utils.clamp`/`mapRange`; Livewire-sicher per data-Flags plus frame-gebuendelter MutationObserver-Adoption, vollstaendig deaktiviert bei `prefers-reduced-motion` (gsap.matchMedia), ohne JS bleibt alles sichtbar. Chat-Senden als Button-in-Button-Pill, Verwalten-Plus mit Rotations-Micro-Interaction. Klassennamen, Geometrie-, Routing- und Livewire-/Alpine-Vertraege unveraendert; `workflow-manager.blade.php`, `workflow-run-preview.blade.php` und `components/workflows/*` blieben unangetastet. | Authentifizierter Browser-Smoke-Test lokal (php artisan serve, Port 8721): Workflows-Liste, Workflow-Manager (6 Listen/11 Task-Karten), eingebettetes Studio mit gemeinsamer Run-Preview (letzter Testlauf geoeffnet) und Copilot-Chat geprueft; JS-Konsole auf allen Screens leer; Reveal-Zyklus ohne haengende `opacity:0`-Elemente; Font per `document.fonts` bestaetigt; Composer-Send aktiviert/deaktiviert korrekt ueber Alpine-Draft. Responsive 390/768/1280 px: kein horizontales Overflow, Metric-Rail 2-/3-spaltig, Task-Bibliothek-Launcher dockt mobil korrekt. Einschraenkung: Pixel-Screenshots nur fuer Login und Workflows-Liste moeglich, danach lieferte das eingebettete Browser-Panel kein Compositing mehr — Struktur-/Style-Pruefung erfolgte per Computed Styles. Tests: UI-/Preview-Suite 33 Tests/307 Assertions und Copilot-Suite 153 Tests/1271 Assertions gruen (SQLite in-memory); Vite-Produktionsbuild (nur bekannte Bundle-Hinweise) und Blade-Cache gruen; `git diff --check` sauber. Temporaerer tokengebundener Dev-Login (Route + Login-Link, nur APP_ENV=local) nach dem Test rueckstandsfrei entfernt. | Pixel-Sichtpruefung im sichtbaren Browser-Panel oder echten Chrome nachholen (Panel-Sitzung ist noch angemeldet, `php artisan serve` auf 8721 laeuft); bei Bedarf Glow-/Schatten-Intensitaeten feinjustieren. |
| 2026-07-27 | Codex | verifiziert | Workflow-Manager, Studio/Testlauf, gemeinsame Run-Preview, pausierter Task-Editor und Chatbot verwenden jetzt ein gemeinsames, Skiper-UI-inspiriertes Blade-/Alpine-Designsystem ohne zusaetzliche React-Laufzeit. `Eine Task` bleibt primaer, Task-Navigation sichtbar, Browser und Diagnosewerkzeuge sind klar gruppiert. Der Chat dockt ab 1140 px korrekt, respektiert die Leseposition, bietet `Neueste Aktivitaet`, trennt Audio-Vorbereitung von echter Wiedergabe und ist auch im eigenstaendigen Studio verfuegbar. Runtime-, Routing- und Force-Stop-Vertraege blieben unveraendert. | SQLite in-memory: 64 relevante Unit-/Featuretests mit 582 Assertions gruen. Blade-Cache, Vite-Produktionsbuild und `git diff --check` erfolgreich; Reduced-Motion-, Fokus-/ARIA- und responsive Markup-Pruefungen enthalten. Der Build meldet nur bekannte Hinweise zu Browserslist-Daten, einem zur Laufzeit aufgeloesten Bild und grossen Pagebuilder-Bundles. | Authentifizierten visuellen Browser-Smoke-Test bei 390, 768 und mindestens 1280 px nachholen; die lokale und die vorhandene Chrome-Sitzung standen nur auf der Login-Seite. |
| 2026-07-26 | Codex | verifiziert | Browser-Session-Laden und -Speichern sind interne workflowweite Runtime-Funktionen. Standard: beide aktiv; in den Workflow-Einstellungen einzeln oder gemeinsam deaktivierbar. Optionaler Session-Key teilt Sessions workflowuebergreifend, sonst gilt `workflow-{id}-person-{id|null}`. Ohne gespeicherte Session oeffnet nur die Fallback-URL. Alte sichtbare Lade-/Speicherkarten migrieren beim Oeffnen oder Start inklusive URL, Key, Domain, Fenster und Label in die Workflow-Einstellungen und werden routensicher entfernt; die Loesch-Task bleibt bewusst sichtbar. Personen- und Haupt-Verifikationskonto-Sessions werden verschluesselt getrennt persistiert. Server- und ClientController-Pfade sowie Remote-Payload-Uebergabe sind umgesetzt. | PHP gezielt mit SQLite: 32 Tests/623 Assertions; Node: 11 Tests; Rust: 16 Tests; Blade-Cache und Vite-Build erfolgreich; `cargo fmt --check` und `git diff --check` gruen. Client-Runtime synchronisiert, SHA-256 der vier geaenderten Node-Dateien identisch; Runtime-Hash `c32072831a9575a9651e314facba7f2689bb03b5f82c13e5eb53f1cc9e09252d` ueber 65 Dateien. | Angemeldeten visuellen Smoke-Test im Workflow-Manager und einen echten Webmail-Lauf mit gemeinsamem Key ueber zwei Workflows durchfuehren. |
...
| 2026-07-30 | Claude | in_arbeit | **Spur W — PWA-Installation und Web-Push aus RailTime nach FollowFlow portieren.** Quelle ist ausschliesslich lesend `C:\xampp\htdocs\RailTime\App`; RailTime wird nicht veraendert. Umfang: VAPID-Auto-Provisionierung, Abo-Verwaltung mit verschluesselten Endpunkten, Zustellung ueber `laravel-notification-channels/webpush`, Retry-Job, Kategorie-Praeferenzen, Service Worker, Web-App-Manifest, Icon-Route mit Fallback-PNG sowie eine eigene Seite zum Installieren, Aktivieren, Steuern und Testen. Beansprucht sind die in der Spurentafel unter **W** gelisteten, ueberwiegend **neuen** Dateien. **Abgrenzung zu Codex' App-Shell-Redesign vom selben Tag:** ich fasse weder Sidebar, Topbar, Workflow-Views, gemeinsame UI-Komponenten noch App-Shell-CSS an. Es bleiben genau drei additive Zeilenbereiche in Codex' Anspruch: (1) `resources/views/layouts/master.blade.php` — eine Zeile `@include('layouts.pwa-head')` im `<head>`; (2) `resources/views/layouts/master-without-nav.blade.php` — dieselbe Zeile; (3) `resources/js/app.js` — `import { registerFactoryPushSettings, registerFactoryPwaInstall, setupFactoryPwa } from './pwa';` plus die drei Aufrufe neben den bestehenden Alpine-Registrierungen. Codex: bitte diese Zeilen beim Shell-Umbau **uebernehmen statt zurueckbauen**; ohne sie faellt Push und Installation still aus. `resources/views/layouts/metahead.php` wird bewusst **nicht** angefasst (siehe Risiko-Spalte). | Abgeschlossen — Ergebnis, Testkommandos und Risiken stehen im `verifiziert`-Eintrag weiter unten. Die Einbindereihenfolge wurde dort noch praezisiert: `layouts.pwa-head` muss **vor** `layouts.metahead` stehen. | Erledigt, siehe Folgeeintrag. |

| 2026-07-30 | Claude | verifiziert | **Spur W abgeschlossen — PWA-Installation und Web-Push aus RailTime portiert** (Spur danach `frei`). Neu: `composer require laravel-notification-channels/webpush:^9.0` (bewusst **nicht** RailTimes `^11.0` — das verlangt Laravel 12, hier laeuft 10.48; `^9.0` zieht `minishlink/web-push ^9`). `config/webpush.php`; `app/Support/Push/{WebPushConfiguration,VapidAutoProvisioner,WebPushTransport,WebPushReport,PushCategory,PushDelivery}.php`; `app/Support/Pwa/PwaIcon.php`; `app/Models/{PushSubscription,NotificationPreference}.php`; `app/Notifications/FactoryWebPushNotification.php`; `app/Http/Controllers/{PushSubscriptionController,PwaIconController}.php`; `app/Http/Requests/StorePushSubscriptionRequest.php`; `app/Rules/{PushEndpoint,WebPushKey}.php`; `app/Listeners/HandleWebPush{Sent,Failed}.php`; `app/Jobs/RetryWebPushDelivery.php`; `app/Livewire/Settings/{PushSettings,PushSettingsPage}.php` mit zwei Blades; `resources/views/layouts/pwa-head.blade.php`; `resources/js/pwa.js`; `public/service-worker.js`; `public/manifest.webmanifest`; `public/icons/{pwa-192,pwa-512,apple-touch-icon-180}.png`; Migration `2026_07_30_090000_create_web_push_tables`. Additiv: `User` (Trait `HasPushSubscriptions`, `notificationPreferences`, `wantsWebPush`, `enableDefaultPushPreferences`, `routeNotificationForWebPush`, `deletePushSubscription`), `EventServiceProvider` (zwei Listener), `RouteServiceProvider` (Limiter `push-subscriptions` 30/min, `push-test` 3/10min), `routes/web.php` (oeffentliche Icon-Route, `/app-installation`, fuenf `settings/push/*`-Routen ausserhalb `role:admin`). **Bewusste Abweichungen zu RailTime:** (1) `content_encoding` bleibt String — das Enum `Minishlink\WebPush\ContentEncoding` existiert erst ab web-push 10 und wuerde hier beim Versand einen TypeError werfen. (2) `WEBPUSH_ENABLED`/`WEBPUSH_TEST_ENABLED` sind per Default **an**, weil sich die VAPID-Schluessel selbst provisionieren und ein Push nur Geraete erreicht, die sich ausdruecklich angemeldet haben. (3) Kategorien sind `workflows`/`copilot`/`system` statt RailTimes `messages`/`chat`/`calls`; `PushDelivery` bleibt generisch und ist noch an keinen Workflow-Ausloeser verdrahtet. (4) Keine Sprachdateien — FollowFlow hat kein `lang/`, die Texte stehen als deutsche Literale in Komponente und Blade. **Befund waehrend der Browser-Pruefung:** `layouts/metahead.php` heisst `.php` statt `.blade.php` und wird deshalb von der PHP- statt der Blade-Engine gerendert; das dortige `<link rel="manifest" href="{{ asset(...) }}">` landet **woertlich** im HTML. Da Browser nur das **erste** Manifest auswerten, haette das die Installierbarkeit blockiert. Geloest ohne Eingriff in metahead: `layouts.pwa-head` wird **vor** `layouts.metahead` eingebunden. | `DB_CONNECTION=sqlite DB_DATABASE=:memory: php artisan test tests/Unit/PwaIconFallbackTest.php tests/Unit/WebPushValidationRulesTest.php tests/Feature/PushSubscriptionApiTest.php tests/Feature/PwaFrontendTest.php tests/Feature/FactoryWebPushNotificationTest.php` -> **36 Tests, 142 Assertions, gruen**. Regression `... php artisan test --filter="Profile\|Authentication\|ApiToken\|Registration\|DeleteAccount\|UpdatePassword\|Workflow"` -> **354 gruen**, 5 uebersprungen, 2 Fehler — beide **vorbestehend und nicht aus Spur W**, per Sondierungstest belegt: `DeleteAccountTest` scheitert an `$user->fresh()`, das die SoftDeletes-Scope umgeht (Loeschung selbst funktioniert, Nutzer ist `trashed`); `RegistrationTest` erwartet 404 auf `/register`, wo `routes/web.php:123` bewusst nach `/login` umleitet (302). `php -l` fuer alle 24 neuen/geaenderten PHP-Dateien sauber; `php artisan migrate --force` gegen MySQL durchgelaufen; `npm run build` erfolgreich, `factory:pwa-state-changed`, `ff-service-worker-url` und `ff-push-account-binding` im Bundle `public/build/js/app2.js` nachgewiesen; `git diff --check` ohne Inhaltsfehler. **Live-Browser (php artisan serve 8721, angemeldet mit einem danach wieder geloeschten lokalen Testkonto):** Service Worker aktiv mit Scope `http://localhost:8721/`, Skript `service-worker.js`; erstes `<link rel="manifest">` ist das neue, liefert 200 `application/manifest+json` mit `display: standalone`; alle vier Icons (`pwa-192`, `pwa-512`, `pwa-maskable-512`, `push-badge-96`) liefern 200 `image/png`; `/settings/push/status` meldet angemeldet `ready: true` mit allen drei Kategorien; `/app-installation` rendert beide Alpine-Komponenten und schaltet korrekt in den Zustand "Benachrichtigungen sind blockiert" (das Browser-Panel hat Benachrichtigungen gesperrt), Abo- und Test-Knopf bleiben dabei verborgen. VAPID-Selbstprovisionierung geprueft: `diagnostics()` meldet `ready: true`, Schluessel liegen in `storage/app/private/webpush-vapid.json` — die Datei ist ueber `storage/app/.gitignore` ignoriert und nicht getrackt. | **Nicht verifiziert: eine echte Push-Zustellung Ende-zu-Ende.** Dafuer braucht es einen Browser, der Benachrichtigungen erlaubt; das eingebettete Panel hat sie auf `denied`. Der Weg bis zur Uebergabe an `minishlink/web-push` ist getestet, der Netzwerkweg zu FCM/Mozilla/Apple nicht. Ebenfalls offen: (a) Pixel-Screenshots — das Browser-Panel liefert wie schon am 2026-07-27 kein Compositing; (b) `PushDelivery` ist an keinen Workflow-Ausloeser verdrahtet — bis dahin kommt ausser der Testbenachrichtigung nichts an; (c) fuer den Produktivbetrieb muss ein Queue-Worker laufen (`QUEUE_CONNECTION=database`), sonst bleiben Pushes liegen; (d) `storage/app/private` muss auf Plesk beschreibbar sein, sonst meldet die Diagnose `auto_provision_failed`. **Fuer Codex:** die drei Verdrahtungszeilen in `master.blade.php`, `master-without-nav.blade.php` und `app.js` beim Shell-Umbau uebernehmen; `layouts/metahead.php` gehoert zu deiner Spur und sollte bei der Gelegenheit in `metahead.blade.php` umbenannt werden — heute landen dort alle `{{ asset(...) }}` woertlich im HTML. Naechster Schritt: Nav-Eintrag auf `/app-installation` in der neuen Shell und Verdrahtung von `PushDelivery` an Workflow-Ende und Copilot-Checkpoint. |

| 2026-07-30 | Codex | verifiziert | **AiUserFactory-App-Shell und Workflow-Oberflaechen an RailTime/App angeglichen; RailTime blieb read-only.** Der authentifizierte Shell besitzt jetzt eine gemeinsame 70-px-Topbar, eine semantische 72/272-px-Desktop-Sidebar, einen 300-px-Mobile-Drawer mit Backdrop/Escape sowie einen einzigen Inhaltswrapper fuer Blade und Full-Page-Livewire. Wiederverwendbare Navigation-, Topbar-, Page-Header- und Surface-Komponenten ersetzen doppelte Wrapper. Dashboard, Profil, Einstellungen und ClientController-Seiten nutzen denselben visuellen Vertrag. Workflow-Liste und Manager wurden fuer Smartphone/Tablet neu geordnet; das Studio besitzt 44-px-Touchziele, niedrige-Viewport-Regeln und einen mobilen Kartenmodus. Test-Workbench und Standalone-Studio liegen mit `z-70` ab `top:0` bewusst ueber Sidebar/Topbar (`z-30`/`z-40`); der Chat bleibt bei `z-90` und oeffnet im mobilen Studio nicht automatisch. Ein parallel eingefuegter, HTML-unsicherer Chat-Kommentar wurde korrigiert, damit das Alpine-Root-Attribut nicht abgeschnitten wird. | `npm run build` erfolgreich; `php artisan view:cache`, `node --check resources/js/app-shell.js`, `php -l app/Livewire/Tools/Chatbot.php` und `git diff --check` gruen. UI-Markup: **11 Tests/217 Assertions**; Workflow-Studio mit SQLite: **28 Tests/209 Assertions**. Authentifizierter In-App-Browser-Smoke-Test bei 320x640, 390x844, 768x900, 1024x900 und 1440x900: kein horizontaler Seitenoverflow, korrekter 1024-px-Shell-Wechsel, Mobile-Drawer per Burger/Escape, Workflow-Tabelle als Karten, Testfenster bei `[0,0]`/`z-70`, sichtbare Touchziele je 44 px und mobiles Studio ohne automatisch geoeffneten Chat. | RailTime/App wurde nur gelesen und ist unveraendert. Bestehende Build-Hinweise bleiben: veraltete Browserslist-Daten, `/site-images/icon-bonus.png` wird zur Laufzeit aufgeloest und der Pagebuilder-Chunk liegt ueber 500 kB. Die Windows-Computer-Use-Integration konnte wegen ihrer URL-Policy nicht fuer den lokalen Firefox-Test genutzt werden; die sichtbare Abnahme erfolgte stattdessen vollstaendig im In-App-Browser. Kein echter externer Workflow/OpenRouter-Lauf und kein Deployment wurden in diesem UI-Auftrag gestartet. |

| 2026-07-30 | Claude | verifiziert | **Spur W in Produktion in Betrieb genommen (factory.follow-flow.de, Plesk).** Zugriff ueber SSH mit vorhandenem Schluessel; die Chrome-Erweiterung war nicht verbunden, deshalb kein Panel-Weg. Vorgefunden: Code und Assets waren bereits ausgerollt, Migration lief, aber **kein Queue-Worker fuer diese Instanz** — der einzige laufende `queue:work` gehoerte `app.rail-time.de`. Da `FactoryWebPushNotification` `ShouldQueueAfterCommit` ist, waere jeder Push liegengeblieben. Zuerst ein eigenes Supervisor-Programm angelegt, dann beim Nachfassen festgestellt, dass die **Plesk-Laravel-Erweiterung** bereits einen Worker verwaltet (`PLESK_EXT_LARAVEL_QUEUE_WORKER_ENABLED=true` in der Prozessumgebung). Das Supervisor-Programm deshalb wieder **entfernt** — zwei Verwalter fuer dieselbe Queue waeren unnoetig und im Plesk-Panel unsichtbar. VAPID-Schluessel als Vhost-Benutzer provisioniert (`storage/app/private/webpush-vapid.json`, Modus 0600, git-ignoriert). Ausserdem `.env`: `APP_DEBUG=true` → `false` (Backup `.env.bak-2026-07-30-appdebug`), weil Ignition sonst bei jedem Fehler Stacktrace samt `.env`-Inhalt oeffentlich ausliefert; `APP_ENV` blieb auf Wunsch unveraendert `local`. | Watchdog-Nachweis: Plesk-Worker gezielt beendet → **2 Sekunden spaeter neu gestartet**; ueber 70 s stabil genau **ein** Worker, keine Akkumulation. Queue-Durchlauf mit echtem `RetryWebPushDelivery`-Job: eingereiht → nach 10 s `offen: 0`, `failed_jobs` unveraendert bei 8 (eigener Testfehlschlag Zeile 39 wieder entfernt). `WebPushConfiguration::diagnostics()` auf dem Server: `ready: true`, keine Issues. Ausgehendes HTTPS zu `fcm.googleapis.com`, `updates.push.services.mozilla.com`, `web.push.apple.com` erreichbar (404/406/405 = Verbindung steht). DocumentRoot zeigt auf `public/`, damit Scope `/`. Live-Browser auf `https://factory.follow-flow.de/login`: Service Worker **aktiv**, Scope `https://factory.follow-flow.de/`, erstes `<link rel="manifest">` ist das neue (200, `application/manifest+json`, `display: standalone`), alle vier Icons 200 `image/png`, `isSecureContext: true`. Nach der `.env`-Aenderung `optimize:clear` und Erreichbarkeit erneut geprueft (302 → /login, /login 200). | **Nicht verifiziert: eine echte Push-Zustellung an ein Geraet** — dafuer braucht es einen Browser mit erlaubten Benachrichtigungen und ein angemeldetes Konto; auf Produktion existiert noch kein einziges Abo (`push_subscriptions: 0`). **Befund fuer Codex' Spur, in Produktion bestaetigt:** `layouts/metahead.php` gibt seine `{{ asset(...) }}`-Ausdruecke woertlich aus, das dortige zweite `<link rel="manifest">` steht als Literal im HTML. Es ist unschaedlich, solange `layouts.pwa-head` **davor** eingebunden bleibt. Weiter offen: `PushDelivery` ist an keinen Workflow-Ausloeser verdrahtet; kein Nav-Eintrag auf `/app-installation`. Betriebshinweis: `storage/app/private/webpush-vapid.json` ist der einzige Ort der VAPID-Schluessel — geht die Datei verloren, muessen sich alle Geraete neu anmelden. |
| 2026-07-30 | Claude | verifiziert | **Spur X abgeschlossen — Laravel Reverb als Echtzeit-Transport, lokal und in Produktion** (Spur danach `frei`). Auftrag war ausdruecklich „ueber Ports, keine weiteren Subdomains". Umsetzung genau so: `laravel/reverb ^1.11` (unterstuetzt `illuminate/* ^10.47`, passt zu 10.48.28 — RailTimes `^11`-Linie waere Laravel 12), `config/reverb.php`, `reverb`-Verbindung in `config/broadcasting.php`, `BroadcastServiceProvider` aktiviert, Echo-Client in `resources/js/bootstrap.js` (nur wenn `VITE_REVERB_APP_KEY` gesetzt ist — sonst laufen endlose Reconnect-Versuche), Diagnose-Ereignis `App\Events\RealtimePing` auf privatem Benutzerkanal plus Befehl `php artisan realtime:ping`. Server: eigenes Supervisor-Programm `followflow-reverb` auf **127.0.0.1:8081** (8080 gehoert `app.rail-time.de`) und ein neues `/var/www/vhosts/system/factory.follow-flow.de/conf/vhost_ssl.conf` nach RailTime-Vorbild, das `/app` (WebSocket) **und** `/apps` (Pusher-HTTP-API, ueber die Laravel selbst einliefert) auf den Loopback-Port proxyt. Nach aussen ist damit **kein zusaetzlicher Port offen und keine Subdomain angelegt**: der Browser verbindet sich auf 443 gegen `factory.follow-flow.de`. Produktions-`.env` bekam eigene, frisch erzeugte `REVERB_APP_ID/KEY/SECRET` (nicht die lokalen), Backup `.env.bak-2026-07-30-reverb`. **`ShouldBroadcastNow` statt `ShouldBroadcast`** fuer das Diagnose-Ereignis ist Absicht: mit Queue misst der Test den Worker statt den Transport — genau darueber ist der erste lokale Durchlauf gestolpert. | `DB_CONNECTION=sqlite DB_DATABASE=:memory: php artisan test tests/Feature/RealtimeBroadcastWiringTest.php` -> **8 Tests, 21 Assertions, gruen** (Verbindungskonfiguration, Provider und `broadcasting/auth`-Route, eigener Kanal erlaubt, **fremder Kanal 403**, Gast 403, Ereignisvertrag, Befehl, JS-Verdrahtung). Dabei zwei echte Testfallen gefunden und dokumentiert: in der Testumgebung greift der `null`-Broadcaster und beantwortet **jede** Autorisierung mit 200; und `Broadcast::channel()` registriert auf der beim Booten aufgeloesten Treiber-Instanz, nach einem Treiberwechsel muss `routes/channels.php` erneut geladen werden. **Lokaler Ende-zu-Ende-Lauf:** Reverb auf 8081, angemeldeter Browser, `Echo.private(...)` `subscribed: true`, `realtime:ping` → Ereignis im Browser angekommen. **Produktions-Ende-zu-Ende-Lauf ohne App-Login**, ueber die dokumentierte Pusher-Signatur: `wss://factory.follow-flow.de/app/<key>` liefert **HTTP 101 Switching Protocols** mit `X-Powered-By: Laravel Reverb`, danach `pusher:connection_established`, mit serverseitig berechnetem HMAC `pusher_internal:subscription_succeeded` auf `private-App.Models.User.1`, und nach `php artisan realtime:ping 1` kam **`realtime.ping` mit korrektem Payload im Browser an**. `/apps/<id>/channels` antwortet mit Reverbs `401 Authentication signature invalid.` statt Laravel-HTML — der Proxy trifft also Reverb. `apachectl -t`: Syntax OK. `supervisorctl status`: `followflow-reverb` und `railtime-reverb` RUNNING; `systemctl is-enabled supervisor` = enabled, ueberlebt also einen Neustart. Nach dem Deploy `php artisan queue:restart`; danach ueber 70 s stabil genau ein Worker. | **Reverb ist bisher reiner Transport**: ausser `RealtimePing` sendet die Anwendung **nichts** — `grep -rl ShouldBroadcast app/` findet nur diese eine Datei. Vor der Umstellung stand `BROADCAST_DRIVER=log`, ein Regressionsrisiko fuer bestehendes Verhalten gibt es deshalb nicht. Offen und bewusst nicht angefangen: fachliche Ereignisse (Workflow-Fortschritt, Copilot-Checkpoints) an Reverb haengen — das beruehrt `WorkflowExecutionService` und die Vorschau, also fremde Spuren. Weitere Hinweise: `laravel-echo@2.4.0` verlangt laut `engines` Node ≥ 20, der Server hat 18.19.1 — Installation und Build laufen trotzdem durch (nur `EBADENGINE`-Warnung), bei kuenftigen Echo-Updates aber im Auge behalten. `composer install` wurde auf dem Server bewusst wieder mit `--no-dev` gefahren, damit der schlanke Ausgangszustand erhalten bleibt. `vhost_ssl.conf` ist eine manuell gepflegte Datei; wer die Domain in Plesk grundlegend umkonfiguriert, sollte pruefen, dass sie weiterhin eingebunden ist (`Include` in `conf/httpd.conf`). Naechster Schritt: entscheiden, welche Ereignisse live gehen sollen, dann in einer eigenen Spur verdrahten. |

| 2026-07-30 | Claude | verifiziert | **Abhaengigkeiten auf den erreichbaren Stand von RailTime gebracht, ohne Framework-Sprung.** `composer update` innerhalb der bestehenden Majors: **84 Pakete**, u. a. `laravel/framework` 10.48.28 → **10.50.2**, `livewire/livewire` 3.5.20 → 3.8.2, `laravel/fortify` 1.25.4 → 1.36.2, `guzzle` 7.9.2 → 7.15.2, `intervention/image-laravel` 1.5.4 → 1.5.9, `spatie/laravel-activitylog` 4.10.1 → 4.12.3 (jetzt gleicher Stand wie RailTime), `dompdf` 3.1.0 → 3.1.6. `npm update` innerhalb der Constraints: **35 direkte Pakete**, u. a. Alpine 3.14.9 → 3.15.12 samt aller Plugins, `axios` 1.17 → 1.19, `laravel-vite-plugin` 1.2 → 1.3. Keine Constraint- oder Major-Aenderung, keine Codeaenderung. **Bewusst NICHT gemacht: der Framework-Sprung auf Laravel 12.** Begruendung im Bericht (siehe naechste Spalte): es gibt keine Drittanbieter-Blockade, aber das Upgrade fasst `composer.lock`, `config/*` und potenziell jede Blade-Datei an, waehrend laut Spurentafel mehrere Agents gleichzeitig im Arbeitsbaum arbeiten. | Eigene Suiten nach dem Update: `php -d max_execution_time=0 vendor/bin/phpunit` fuer die fuenf PWA-/Push-/Reverb-Dateien -> **44 Tests, 163 Assertions, gruen**. Anschliessend **jede** Testdatei einzeln durchlaufen (`tests/Feature/*.php` und `tests/Unit/*.php`): **keine einzige Zeitueberschreitung**, und nur die drei bereits dokumentierten Altfehler — `DeleteAccountTest` (`$user->fresh()` umgeht die SoftDeletes-Scope), `ExampleTest` (unveraenderter Laravel-Stub aus dem Initial-Commit, erwartet 200 auf `/`, das hinter Auth liegt), `RegistrationTest` (erwartet 404 auf `/register`, `routes/web.php` leitet bewusst um). `WorkflowCopilotSupervisorTest` war im Sammellauf einmal rot und ist bei erneuter isolierter Ausfuehrung **28 Tests / 231 Assertions gruen** — die Datei wurde waehrenddessen von einem anderen Agent bearbeitet. `npm run build` gruen; `factoryPushSettings`, `factoryPwaInstall`, `ff-service-worker-url` und `reverb` weiterhin im Bundle nachgewiesen. | **Befund zur Gesamtsuite:** `php artisan test` bricht lokal mit „Maximum execution time of 40 seconds exceeded" ab. Das ist **kein Hänger**, sondern die kumulative Prozesszeit — einzeln laeuft jede Datei in Sekunden durch. Wer die Gesamtsuite braucht, muss `max_execution_time` in der XAMPP-`php.ini` anheben; `php -d` allein genuegt nicht, weil der Testrunner einen Unterprozess startet. **Verbleibendes Delta zu RailTime ist ausschliesslich der Framework-Major** und was daran haengt: Laravel 12, Jetstream 5, Sanctum 4, Web-Push 11, Symfony 7, Carbon 3, PHP `^8.2`. Drittanbieter wurden gegen Packagist geprueft und blockieren nicht (`larapex-charts` 9 unterstuetzt `^9…^12`, `laravel-dompdf` v3 `^9…^13`, `activitylog` 4.12 `^8…^13`; `panther`, `php-webdriver`, `bdi`, `paypal`, `barcode` sind framework-unabhaengig). Die L11-Skelettumstellung ist optional, alle vier Kernel-/Handler-Dateien duerfen bleiben. Vollstaendige Analyse inklusive ClientController-Teil: `.lmzdev/artifacts/reports/stack-abgleich-und-clientcontroller-kommunikation-2026-07-30.md`. Naechster Schritt: Framework-Upgrade als eigenes Paket einplanen, wenn die Spurentafel leer ist. |
...
| **W — PWA-Installation & Web-Push** | Ausschliesslich **neue** Dateien: `config/webpush.php`, `app/Support/Push/*`, `app/Support/Pwa/PwaIcon.php`, `app/Models/PushSubscription.php`, `app/Models/NotificationPreference.php`, `app/Notifications/FactoryWebPushNotification.php`, `app/Http/Controllers/PushSubscriptionController.php`, `app/Http/Controllers/PwaIconController.php`, `app/Http/Requests/StorePushSubscriptionRequest.php`, `app/Rules/PushEndpoint.php`, `app/Rules/WebPushKey.php`, `app/Listeners/HandleWebPush*.php`, `app/Jobs/RetryWebPushDelivery.php`, `app/Livewire/Settings/PushSettings.php`, `resources/views/livewire/settings/push-settings.blade.php`, `resources/views/layouts/pwa-head.blade.php` (neu), `resources/js/pwa.js` (neu), `public/service-worker.js`, `public/manifest.webmanifest`, neue Migration, neue Tests. Dazu **additiv** in gemeinsamen Dateien: `app/Models/User.php`, `app/Providers/EventServiceProvider.php`, `app/Providers/RouteServiceProvider.php`, `routes/web.php`, `composer.json`/`composer.lock` | Portierung des RailTime-Verfahrens (`C:\xampp\htdocs\RailTime\App`, read-only) nach FollowFlow: Web-Push inkl. VAPID-Auto-Provisionierung, Service Worker, Manifest, Installations-/Steuerungs-UI | `frei` — erledigt und verifiziert; **Ueberschneidung mit Codex' App-Shell-Anspruch vom 2026-07-30 an genau drei Stellen: `resources/views/layouts/master.blade.php` und `resources/views/layouts/master-without-nav.blade.php` (je eine `@include('layouts.pwa-head')`-Zeile, die zwingend VOR `@include('layouts.metahead')` stehen muss) sowie `resources/js/app.js` (ein `import` plus drei Aufrufe). Codex bitte diese Zeilen samt Reihenfolge in die neue Shell uebernehmen statt zurueckzubauen — `tests/Feature/PwaFrontendTest.php` pinnt beide Stellen.** Keine Workflow-, Runtime-, Routing- oder Copilot-Vertraege beruehrt. | Claude (erledigt) |

| **X — Reverb / Echtzeit-Transport** | Neu: `config/reverb.php`, `app/Events/*` (Demo-/Testereignis), neue Tests. Additiv in gemeinsamen Dateien: `config/broadcasting.php` (nur neue `reverb`-Verbindung), `config/app.php` (nur `BroadcastServiceProvider` aktivieren), `routes/channels.php`, `resources/js/bootstrap.js` (auskommentierter Echo-Block wird aktiviert), `.env`/`.env.example`, `composer.json`/`composer.lock`, `package.json`/`package-lock.json`. Serverseitig: Supervisor-Programm `followflow-reverb` auf `127.0.0.1:8081` und Apache-Proxy in `vhost_ssl.conf` von `factory.follow-flow.de` | WebSocket-Transport nach RailTime-Vorbild (`app.rail-time.de` nutzt 8080), damit spaetere Live-Updates ohne Neuladen moeglich sind. **Kein** eigener Port nach aussen und **keine** Subdomain: der Client verbindet ueber 443 auf `factory.follow-flow.de`, Apache proxyt auf den Loopback-Port. | `frei` — erledigt und in Produktion verifiziert. **Ueberschneidung mit Codex' App-Shell-Anspruch nur bei `resources/js/bootstrap.js`** (dort wird der bereits vorhandene, auskommentierte Echo-Block aktiviert). Reverb ist reiner Transport; **welche** Ereignisse gesendet werden, ist bewusst ein eigenes, spaeteres Paket und beruehrt Workflow-Dateien fremder Spuren nicht. | Claude (erledigt) |
| **Y — Skill `workflow-from-browser`** | Ausschliesslich ausserhalb von `AiUserFactory/`: `.claude/skills/workflow-from-browser/**` (SKILL.md, drei Referenzen, `scripts/build_workflow_csv.py`, `assets/catalog.json`, `assets/beispiel-suche.json`) | Portale mit dem In-App-Browser erkunden, Selektoren gegen echtes DOM haerten und daraus eine importfertige Workflow-CSV erzeugen — der Teil, den der Copilot mangels Browser nicht leisten kann | `frei` — erledigt und verifiziert. **Keine Datei in `AiUserFactory/` geaendert**, damit kein Anspruch fremder Spuren beruehrt wird. `assets/catalog.json` ist ein Abzug von `WorkflowTaskCatalog` (48 Schluessel, Stand 2026-07-31); waechst der Katalog, muss er neu exportiert werden — das Snippet dazu steht in `references/task-catalog.md`. Verifiziert per Roundtrip: erzeugte CSV → `WorkflowTransferService::importCsv` → 4 Listen/12 Karten korrekt in der DB → Testworkflow wieder geloescht. | Claude (erledigt) |
| **Z — Workflow `instagram-login-session-v2`** | Nur **Datenbankinhalt**: neuer Workflow mit Slug `instagram-login-session-v2` (6 Listen, 15 Karten). Keine Repodatei geaendert. | Ersatz fuer den defekten Altbestand `instagram-login-session-speichern-neu`; alle Selektoren bis zum Absenden am 2026-07-31 live auf instagram.com gemessen | `frei` — erledigt. `WorkflowDefinitionValidator::validate` meldet `valid=true`, 0 Diagnosen (der Altbestand meldet `valid=false`, 2× `invalid_configuration_option`). **Noch nicht ausgefuehrt:** ein echter Testlauf steht aus, weil `persons` id=1 keine Zugangsdaten traegt (`login_username` NULL, `auto_login_enabled=0`). Unverifiziert bleibt der Zustand nach dem Login — Instagram landet laut den Scraper-Logs vom 06.06.2026 haeufig auf `/accounts/onetap/`, deshalb prueft der Workflow den Erfolg ueber das Verschwinden des Loginformulars statt ueber ein Feed-Element. Kanonischer Login-Beleg im Projekt waere das `sessionid`-Cookie (`resources/node/scraper/scrape-instagram.cjs:106-110`); dafuer gibt es im Task-Katalog **keine** Karte — offene Luecke. | Claude (erledigt) |
