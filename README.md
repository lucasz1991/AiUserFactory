...
| **X — Reverb / Echtzeit-Transport** | Neu: `config/reverb.php`, `app/Events/*` (Demo-/Testereignis), neue Tests. Additiv in gemeinsamen Dateien: `config/broadcasting.php` (nur neue `reverb`-Verbindung), `config/app.php` (nur `BroadcastServiceProvider` aktivieren), `routes/channels.php`, `resources/js/bootstrap.js` (auskommentierter Echo-Block wird aktiviert), `.env`/`.env.example`, `composer.json`/`composer.lock`, `package.json`/`package-lock.json`. Serverseitig: Supervisor-Programm `followflow-reverb` auf `127.0.0.1:8081` und Apache-Proxy in `vhost_ssl.conf` von `factory.follow-flow.de` | WebSocket-Transport nach RailTime-Vorbild (`app.rail-time.de` nutzt 8080), damit spaetere Live-Updates ohne Neuladen moeglich sind. **Kein** eigener Port nach aussen und **keine** Subdomain: der Client verbindet ueber 443 auf `factory.follow-flow.de`, Apache proxyt auf den Loopback-Port. | `in_arbeit` seit 2026-07-30 — **Ueberschneidung mit Codex' App-Shell-Anspruch nur bei `resources/js/bootstrap.js`** (dort wird der bereits vorhandene, auskommentierte Echo-Block aktiviert). Reverb ist reiner Transport; **welche** Ereignisse gesendet werden, ist bewusst ein eigenes, spaeteres Paket und beruehrt Workflow-Dateien fremder Spuren nicht. | Claude |
