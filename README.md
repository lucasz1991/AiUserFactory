...
| 2026-07-15 | Codex | in_arbeit | Produktionsaktivierung der serverlokalen Sprache fortgesetzt. Die angemeldete Plesk-Sitzung wurde in Firefox gefunden, kann wegen der URL-Sicherheitsgrenze der Windows-Browsersteuerung aber nicht automatisiert werden. Lokaler SSH-Key, SSH-Config, PuTTY/WinSCP und FileZilla wurden ohne Auslesen von Passwoertern geprueft; kein Zugang fuer `factory.follow-flow.de` vorhanden. Als kontrollierter Ersatz entsteht eine admin-geschuetzte Runtime-Seite, die ausschliesslich das vorhandene gepinnte `scripts/bootstrap-local-assistant-voice.sh` als Domain-Benutzer startet und PID, Log sowie Komponentenstatus sichtbar macht. Claudes paralleler OpenRouter-Bereich bleibt unberuehrt. | Firefox-Fenster `Laravel Toolkit - Plesk Obsidian 18.0.78` erkannt; Computer Use beendet mit `URL policy enforcement is not yet supported for the current Windows browser`; SSH-Port erreichbar, aber kein lokales Benutzerprofil fuer den Host. | Installer-Service, Admin-Routen/-Ansicht und Tests umsetzen; deployen, Lauf explizit starten und danach Whisper/Piper sowie echte Ein-/Ausgabe produktiv pruefen. |
