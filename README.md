...
| 2026-07-15 | Codex | in_arbeit | Kostenfreie serverlokale Sprachein- und -ausgabe fuer den Workflow-Chatbot nach dem Luczor-Muster. Kernpaket umgesetzt: isolierte `ffmpeg`-/Whisper-/Piper-Prozesse mit Status, Locks, Timeouts und Temp-Bereinigung; vorhandene Audio-Endpunkte um `whisper_local`/`piper_local` erweitert; Einstellungen und Chat auf beide Provider vorbereitet; Chat-Autoscroll um zeitversetzte Nachpruefungen ergaenzt. Keine Aenderungen an Claudes Copilot-Supervisor-/Observation-Arbeitsbereich. | Produktiver Port `factory.follow-flow.de:2701` ist von aussen nicht erreichbar. Neue PHP-Dateien und geaenderte Controller sind syntaxgeprueft; Funktions- und UI-Tests stehen noch aus. | Status-/Aktivierungsbefehl, Fake-Binary-Tests sowie Linux/Plesk-Bootstrap integrieren und danach den realen Serverpfad verifizieren. |

| 2026-07-15 | Claude | in_arbeit | UI/UX-Optimierung Workflow-Manager-Board, rein Frontend ohne Logikaenderung: (1) Der "+ Task am Listenende"-Button pro Liste wird erst beim Hover ueber der Liste sichtbar (Tastatur-Fokus zeigt ihn weiterhin). (2) Klick-Einfuegen statt Drag-Pflicht: Der Button merkt sich Ziel-Liste und Position, oeffnet die Task-Bibliothek, und ein Klick auf einen Katalogeintrag ruft das bestehende `prepareTaskFromCatalog` fuer genau diese Stelle auf; Drag and Drop bleibt unveraendert erhalten. Bereich: `resources/views/components/workflows/step-card.blade.php` und `resources/views/livewire/admin/network/workflow-manager.blade.php` (nur Alpine/Markup). Keine Ueberschneidung mit Codex' Audio-Paket. | Ausstehend. | Blade-Kompilierung und UI-/Markup-Tests ausfuehren, Eintrag auf verifiziert setzen. |
